...
## 2026-09-16 房間布局與共用戰報（來源檢查點）

`QSAN Actions` 現改為房間畫面，保留現有 D–F 欄選擇契約；更新原有 Apps Script
...
缺失的上游翻譯仍按 TUI 的 fallback 顯示，不宣稱所有擴展詞條已翻譯。

使用者批准檢查點後，Sheets bridge／TUI／TUI 測試目標增量建置成功，前端測試
27/27 通過，TUI `card-text` 與 `log-text` 均通過（各少於 60 秒）。初次前端測試
發現的戰報末列邊界已修正。詳見[驗證報告](../docs/google-sheets-room-validation-20260916.md)。
此版後續已完成線上部署及一局 05P 託管對局，包含 GAME_OVER、主公＋忠臣勝、
正常退出與清理；版面仍有待修問題，見[線上驗收報告](../docs/google-sheets-room-trust-acceptance-20260916.md)。

### 說明、圖片與座位詳情（建置／短測試及腳本部署通過）

Sheets 增量建置 exit 0、前端 30/30 通過；三個改動的 Apps Script 檔案已更新線上，
並讀回核對一致。新版選單及側欄提示已載入，對局內查詢仍待驗收，見
[檢查點報告](../docs/google-sheets-details-validation-20260916.md)。
