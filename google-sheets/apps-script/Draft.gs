...
function integer_(value, minimum) {
  ...
  if (!/^-?[0-9]+$/.test(text)) throw new Error('请输入整数。');
  const n = Number(text);
  if (!Number.isSafeInteger(n) || n < minimum || n > 2147483647) throw new Error('整数超出范围。');
  return n;
}
function boolean_(value) {
  ...
  throw new Error('布林栏位请填 TRUE 或 FALSE。');
}
...
function ordered_(rows, bank, repeats) {
  ...
  rows.forEach((r, index) => {
    ...
    if (!boolean_(r[6])) throw new Error('选到了不可用项目，请重新预检。');
    const raw = String(r[4]).trim();
    const places = raw ? raw.split(',').map(x => integer_(x.trim(), 1)) : [1000000 + index];
    if (!repeats && places.length !== 1) throw new Error('只有目标可以填入多个顺序，例如 1,3。');
    places.forEach(position => items.push({row: r, position: position}));
  });
  items.sort((a, b) => a.position - b.position);
  for (let i = 1; i < items.length; ++i) if (items[i].position === items[i - 1].position) throw new Error('顺序不可重复。');
  return items.map(x => x.row);
}
function draftFromRows_(meta, rows) {
  const selected = bank => ordered_(rows, bank, bank === 'player');
  const only = bank => { const list = selected(bank); if (list.length > 1) throw new Error('此项只能选一个。'); return list[0]; };
  ...
  switch (meta.shape) {
    ...
    case 'option': {
      const row = only('option'); if (!row) throw new Error('请选择一个选项。');
    ...
    case 'assignment': {
      const assignments = selected('assignment').map(r => {
        const role = String(r[5]); if ((meta.roles || []).indexOf(role) < 0) throw new Error('请替玩家选择原生提供的角色。');
    ...
    case 'rearrangement': {
      ...
      selected('rearrange').forEach(r => {
        ...
        if (side !== 'top' && side !== 'bottom') throw new Error('观星侧别请填 top 或 bottom。');
        (side === 'top' ? top : bottom).push(integer_(r[1], 0));
      });
      return {top: top, bottom: bottom};
    }
    case 'distribution': {
      const names = targets(); if (names.length !== 1) throw new Error('每次分牌请选一名接收者。');
      return {cards: cardIds(), target: names[0]};
    }
    case 'general_arrangement': return {order: selected('general').map(r => String(r[1]))};
    default: throw new Error('此互动尚未支援，没有送出任何回复。');
  }
}
function readDraft_() {
  requirePaired_(); const meta = json_('meta', null);
  if (!meta || !decimal_(meta.request_id) || meta.request_id === '0' || meta.shape === 'none') throw new Error('目前没有待回复互动。');
  const count = Number(get_('action_rows', '0'));
  if (!count) throw new Error('互动选项尚未载入，请重新整理。');
