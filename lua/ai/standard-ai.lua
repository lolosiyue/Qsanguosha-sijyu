...
sgs.ai_skill_use_func.NosJujianCard = function(card,use,self)
	...
	if self:getCardsNum("Peach")<1 and self.player:isWounded()
	and self.player:getCards("he"):length()>=3 then
		local cards = self.player:getCards("he")
		cards = sgs.QList2Table(cards)
		self:sortByUseValue(cards,true)
		for _,card in ipairs(cards)do
			if card:getTypeId()==sgs.Card_TypeTrick
			and not isCard("ExNihilo",card,self.player)
			then trick_num = trick_num+1
			elseif card:getTypeId()==sgs.Card_TypeBasic
			then basic_num = basic_num+1
			elseif card:getTypeId()==sgs.Card_TypeEquip
			then equip_num = equip_num+1 end
		end
		local result_class
		if trick_num>=3 then result_class = "TrickCard"
		elseif equip_num>=3 then result_class = "EquipCard"
		elseif basic_num>=3 then result_class = "BasicCard" end

		for _,fcard in ipairs(cards)do
			if result_class and fcard:isKindOf(result_class)
			and not isCard("ExNihilo",fcard,self.player) then
				table.insert(abandon_card,fcard:getId())
				if #abandon_card>=3 then break end
			end
		end

		if #abandon_card>=3 then
	...
	if n>0 then
		...
		if AssistTarget and not AssistTarget:hasSkill("manjuan") and not self:needKongcheng(AssistTarget,true) then
			...
			to = self:findPlayerToDraw(false,n)
		end
		if not to then return end
		local discard = self:askForDiscard("dummyreason",n,n,false,true)
		use.card = sgs.Card_Parse("@NosJujianCard="..table.concat(discard,"+"))
		use.to:append(to)
		return
	end

	if #abandon_card>0 then
		if AssistTarget and not AssistTarget:hasSkill("manjuan") and not self:needKongcheng(AssistTarget,true) then
			to = AssistTarget
		else
			to = self:findPlayerToDraw(false,#abandon_card)
		end
		if not to then return end
		use.card = sgs.Card_Parse("@NosJujianCard="..table.concat(abandon_card,"+"))
		use.to:append(to)
...
sgs.ai_skill_cardask["@nosenyuan-heart"] = function(self,data)
	...
	if self:needToLoseHp(self.player,damage.to,damage.card,true) and not self:hasSkills(sgs.masochism_skill) then return "." end
	if self:isFriend(damage.to) then return end
	if self:needToLoseHp(nil,nil,damage.card) and not self:hasSkills(sgs.masochism_skill) then return "." end
...
function sgs.ai_slash_prohibit.nosenyuan(self,from,to,card)
	...
	if (role=="loyalist" or role=="lord") and sgs.playerRoles.rebel+sgs.playerRoles.renegade==1
...
sgs.ai_need_damaged.nosenyuan = function (self,attacker,player)
	...
	and not attacker:hasSkills(sgs.masochism_skill)
...
sgs.ai_skill_choice.nosxuanfeng = function(self,choices)
	...
	for _,enemy in ipairs(self.enemies)do
		...
		then return "damage"
		elseif not self:slashProhibit(slash,enemy) 
...
nosgongqi_skill.getTurnUseCard = function(self,inclusive)
	...
	if equip_card then
		...
		return sgs.Card_Parse(("slash:nosgongqi[%s:%s]=%d"):format(suit,number,card_id))
	end
end
...
function sgs.ai_cardsview_valuable.nosjiefan(self,class_name,player)
	if player:getMark("NosJiefanFailed-Clear")>5
	or self:getCardsNum("Slash")<1 then return end
	player:addMark("NosJiefanFailed-Clear")
...
sgs.ai_skill_invoke.nosfuhun = function(self,data)
	local target = 0
	for _,enemy in ipairs(self.enemies)do
		if self.player:distanceTo(enemy)<=self.player:getAttackRange()
		then target = target+1 end
	end
...
sgs.ai_skill_invoke.nosqianxi = function(self,data)
	...
	if self:ajustDamage(self.player,target,1,damage.card)>1 then return false end
...
noslijian_skill.getTurnUseCard = function(self)
	if self.player:isNude() or self.room:getAlivePlayers():length()<3
	then return end
...
sgs.ai_choicemade_filter.NosLijianCard = function(self,player,carduse)
...
sgs.ai_skill_use_func.NosRendeCard = function(card,use,self)
	...
	for i = 1,self.player:getHandcardNum() do
		...
		then use.card = sgs.Card_Parse("@NosRendeCard="..h:getId().."+"..cards[1]:getId())
		else use.card = sgs.Card_Parse("@NosRendeCard="..h:getId()) end
		use.to:append(friend)
		return
	end
	if notFound and self.player:isWounded()
	and self:getOverflow()>0 and self.player:getMark("nosrende")<2 then
		...
		for _,p in sgs.qlist(self.room:getOtherPlayers(self.player))do
			if not self:isFriend(p) and hasManjuanEffect(p) then
				...
				for _,h in ipairs(cards)do
					...
					if #to_give>=2-self.player:getMark("nosrende")
					then break end
				end
				if #to_give>0 then
...
sgs.ai_use_revises.nosrende = function(self,card,use)
	if self.player:getLostHp()>1 and self:findFriendsByType(sgs.Friend_Draw)
	and self.player:getMark("nosrende")<2
...
function sgs.ai_slash_prohibit.nosleiji(self,from,to,card)
	...
	if from:getRole()=="rebel" and to:isLord() then
		...
		if not other_rebel and ((from:getHp()>=4 and (getCardsNum("Peach",from,self.player)>0 or from:hasSkills("nosganglie|vsnosganglie"))) or from:hasSkill("hongyan"))
		then return false end
	end

	if hcard>=4 or (hcard>=2 and self:hasSuit("spade",true,to)) or getKnownCard(to,from,"Jink",true)>=1 then return true end
...
sgs.ai_skill_choice.nosguhuo = function(self,choices)
	...
	local nosguhuocard = dummyCard(nosguhuoname)
	if self:getRestCardsNum(nosguhuocard:getClassName(),yuji)<1 and self.player:getHp()>0 then return "question" end
	if nosguhuocard:getClassName()=="AmazingGrace" then return "noquestion" end
	if nosguhuocard:getClassName():match("Slash") then
	...
	if self:isFriend(yuji) or sgs.questioner then return "noquestion"
	elseif self.player:getHp()<self.friends[#self.friends]:getHp()
	then return "noquestion"end
...
sgs.ai_choicemade_filter.skillChoice.nosguhuo = function(self,player,promptlist)
	if promptlist[#promptlist]=="question" then
		...
		if nosguhuocard then
			if self:getRestCardsNum(nosguhuocard:getClassName(),yuji)>0 then
				sgs.updateIntention(player,yuji,80)
				return
			end
		end
...
nosguhuo_skill.getTurnUseCard = function(self)
	...
	local ps = patterns()
	for i=1,9 do
        local name = ps[math.random(1,#ps)]
	...
	for _,h in ipairs(cards)do
		...
		and (can or self:getCardsNum(h:getClassName())>1) then
	...
	for _,h in ipairs(cards)do
		...
		and (h:isKindOf("BasicCard") or h:isNDTrick()) then
         	local dummy = self:aiUseCard(h)
    		if dummy.card then
	         	self.guhuo_to = dummy.to
	           	if h:canRecast() and dummy.to:length()<1 then continue end
	...
	if self:isWeak() then
		local peach_str = sgs.ai_guhuo_card.nosguhuo(self,"peach","Peach")
		if peach_str and math.random(0,#cards)>2 then
	...
	local slash_str = sgs.ai_guhuo_card.nosguhuo(self,"slash","Slash")
	if slash_str and self:slashIsAvailable()
	and math.random(0,#cards)>1 then
...
sgs.ai_use_priority.NosGuhuoCard = 11
	...
	elseif phase==sgs.Player_Play then
		if damage.chain or self.room:getTag("is_chained"):toInt()>0 then
			for _,ap in sgs.qlist(self.room:getAllPlayers())do
				if ap:isChained() and self:isGoodChainTarget(ap,damage.card,nil,damage.damage) then return false end
			end
			return true
		elseif self:getOverflow()>=2 then
			...
			if (damage.card and damage.card:isKindOf("AOE")) or (self.player:hasFlag("ShenfenUsing") and self.player:faceUp()) then
				if damage.to:getNextAlive():objectName()==self.player:objectName() then return true
					...
					local p = damage.to
					repeat
						if self:damageIsEffective(p,damage.card,self.player)
						then
							if self:isFriend(p) then
								dmg_val = dmg_val+1
							else
								if self:cantbeHurt(p,self.player,damage.damage) then dmg_val = dmg_val+1 end
								if self:needToLoseHp(p,self.player,damage.card) then dmg_val = dmg_val+0.5 end
...
sgs.ai_skill_use["@@nosmieji"] = function(self,prompt) -- extra target for Collateral
	...
	self:useCardCollateral(dummyCard("collateral"),dummy_use)
...
local function getNosFenchengValue(self,player)
	...
	if self.player:hasSkills("juece|nosjuece") and self:isEnemy(player)
	and player:getEquips():isEmpty() and player:getHandcardNum()==1 and not self:needKongcheng(player)
	and not (player:isChained() or not self:isGoodChainTarget(player,"F")) then return self:isWeak(player) and 1.5 or 1 end
	if self:isGoodChainTarget(player,"F") or self:needToLoseHp(player,self.player) then return -0.1 end
...
sgs.ai_skill_discard.nosfencheng = function(self,discard_num,min_num,optional,include_equip)
	...
	local juece_effect
	if liru and liru:isAlive() and liru:hasSkills("juece|nosjuece") then juece_effect = true end
	...
	if juece_effect and self:isEnemy(liru) and self.player:getEquips():isEmpty() and self.player:getHandcardNum()==1 and not self:needKongcheng()
		and not (self.player:isChained() or not self:isGoodChainTarget(self.player,"F",liru)) then return {} end
	if self:isGoodChainTarget(self.player,"F",liru) or self:needToLoseHp(self.player,liru) then return {} end
...
function sgs.ai_cardsview_valuable.nosrenxin(self,class_name,player)
	...
	if dying:isLord() and self:isFriend(dying) then return "@NosRenxinCard=." end
	...
	if player:faceUp() then
		for _,c in sgs.qlist(player:getHandcards())do
			if isCard("Peach",c,player) then return nil end
		end
...
sgs.ai_choicemade_filter.cardChosen.nosfankui = function(self,player,promptlist)
	...
	if damage.from then
		...
		if card:isKindOf("Armor") and self:needToThrowArmor(damage.from)
		and self.room:getCardPlace(promptlist[3])==sgs.Player_PlaceEquip
		then intention = -intention
		elseif not self:doDisCard(damage.from,"he") then intention = -intention
		elseif card:isKindOf("EquipCard") and self:hasSkills(sgs.lose_equip_skill,damage.from)
		and self.room:getCardPlace(promptlist[3])==sgs.Player_PlaceEquip
		and damage.from:getEquips():length()>0 then intention = -intention
		elseif sgs.ai_need_damaged.nosfankui(self,damage.from,player)
...
sgs.ai_need_damaged.nosfankui = function (self,attacker,player)
	if not player:hasSkills("nosguicai+nosfankui") then return false end
...
sgs.ai_skill_cardask["@nosguicai-card"]=function(self,data)
	local judge = data:toJudge()
	if self.room:getMode():find("_mini_46") and not judge:isGood()
...
sgs.ai_need_damaged.nosganglie = function(self,attacker,player)
	...
	and not(self:hasSkills(sgs.need_kongcheng.."|buqu",attacker) and attacker:getHandcardNum()>1)
...
sgs.ai_skill_use["@@nostuxi"] = function(self,prompt)
	...
	if zhijiangwei and self:isFriend(zhijiangwei) and zhijiangwei:getHandcardNum()==1 and
		...
		local slash = dummyCard()
		for _,enemy in ipairs(self.enemies)do
			if zhijiangwei:canSlash(enemy,slash) and not self:slashProhibit(slash,enemy,zhijiangwei)
			and self:isGoodTarget(enemy,self.enemies,slash) and self:getDefenseSlash(enemy)<4 then
				isGood = true
			end
		end
	...
	for _,other in sgs.qlist(others)do
		if self:objectiveLevel(other)>=0 and not(other:hasSkill("tuntian") and other:hasSkill("zaoxian",true)) and add_player(other)==2 then
			return ("@NosTuxiCard=.->%s+%s"):format(targets[1],targets[2])
		end
	end

	for _,other in sgs.qlist(others)do
		if self:objectiveLevel(other)>=0 and not(other:hasSkill("tuntian") and other:hasSkill("zaoxian",true)) and add_player(other)==1 and math.random(0,5)<=1 and not self:hasSkills("qiaobian") then
...
sgs.ai_skill_invoke.nosluoyi = function(self,data)
	...
	for _,card in ipairs(cards)do
		...
		if card:isKindOf("Duel") then
			for _,enemy in ipairs(self.enemies)do
				if self:getCardsNum("Slash")>=getCardsNum("Slash",enemy) and self:isGoodTarget(enemy,self.enemies,card)
...
function sgs.ai_cardneed.nosluoyi(to,card,self)
	...
	self:sort(self.enemies,"defenseSlash")
	for _,enemy in ipairs(self.enemies)do
		if to:canSlash(enemy) and not self:slashProhibit(slash,enemy)
		and self:slashIsEffective(slash,enemy) and self:getDefenseSlash(enemy)<=2 then
			target = enemy
			break
		end
	end
...
sgs.ai_skill_invoke.nosyiji = function(self)
	...
	for _,ap in sgs.qlist(self.room:getAlivePlayers())do
		if self.player:hasFlag("ShenfenUsing")
		then Shenfen_user = ap break end
	end
	if self.player:getHandcardNum()<2 then return true end
	for _,friend in ipairs(self.friends)do
		if hasManjuanEffect(friend)
		or Shenfen_user and Shenfen_user:objectName()~=friend:objectName() and friend:getHandcardNum()>=4
		or self:needKongcheng(friend,true)
...
sgs.ai_skill_askforyiji.nosyiji = function(self,card_ids,tos)
	local cards = {}
	...
	for _,p in sgs.list(tos)do
		if p:hasFlag("ShenfenUsing") then Shenfen_user = p break end
	end
	for _,c in ipairs(self:poisonCards(cards))do
		local id = c:getEffectiveId()
		if self.player:getEquipsId():contains(id)
		or self.player:getJudgingAreaID():contains(id) then
			for _,p in sgs.list(tos)do
				if self:isFriend(p) then return p,id end
			end
			for _,p in sgs.list(tos)do
				if not self:isEnemy(p) then return p,id end
			end
			for _,p in sgs.list(tos)do
				return p,id
			end
			...
			for _,p in ipairs(self.enemies)do
				if self:isEnemy(p) and tos:contains(p)
				then return p,id end
			end
			for _,p in sgs.list(tos)do
				if not self:isFriend(p) then return p,id end
			end
		end
	end
	if Shenfen_user then
		if self:isFriend(Shenfen_user) then
			if Shenfen_user~=self.player then
				for _,id in ipairs(card_ids)do
			...
			if self.player:getHandcardNum()<self:getOverflow(false,true)
			then return nil,-1 end
			local card,friend = self:getCardNeedPlayer(cards)
			if card and friend and friend:getHandcardNum()>=4
			and tos:contains(friend) then return friend,card:getId() end
		end
	elseif self.player:getHandcardNum()<=2
	then return nil,-1 end
	local new_friends = {}
	local CanKeep = self.player:hasShownRole()
	for _,friend in ipairs(self.friends)do
		if tos:contains(friend) and not hasManjuanEffect(friend)
		and not self:isLihunTarget(friend) and not self:needKongcheng(friend,true)
		and (not Shenfen_user or friend==Shenfen_user or friend:getHandcardNum()>=4) then
			if friend~=self.player then table.insert(new_friends,friend)
			else CanKeep = true end
		end
	end
	if #new_friends>0 then
		local card,target = self:getCardNeedPlayer(cards)
		if card and target and table.contains(new_friends,target) then
			return friend,card:getId()
		end
		if Shenfen_user and self:isFriend(Shenfen_user)
		then return Shenfen_user,cards[1]:getId() end
		self:sort(new_friends,"defense")
		self:sortByKeepValue(cards,true)
		return new_friends[1],cards[1]:getId()
	elseif CanKeep then
		return nil,-1
	else
		local other = {}
		for _,p in sgs.list(tos)do
			if not(self:isEnemy(p) or self:isLihunTarget(p) or p:hasSkill("lihun"))
			then table.insert(other,p) end
		end
		if #other<1 then return nil,-1 end
...
function sgs.ai_skill_suit.nosfanjian(self)
	...
	if self.player:hasSkill("hongyan") and (maxsuit==sgs.Card_Spade or suit==sgs.Card_Spade)
	then return sgs.Card_Heart end
	if maxsuit then
		if self.player:hasSkill("hongyan") and maxsuit==sgs.Card_Spade
		then return sgs.Card_Heart end
		return maxsuit
	else
		if self.player:hasSkill("hongyan") and suit==sgs.Card_Spade
		then return sgs.Card_Heart end
		return suit
	end
end
...
nosguose_skill.getTurnUseCard=function(self,inclusive)
	...
	return sgs.Card_Parse("indulgence:nosguose[no_suit:0]="..card:getEffectiveId())
end
...
qingnang_skill.getTurnUseCard = function(self)
	...
	return sgs.Card_Parse("@QingnangCard="..cards[1]:getId())
end
...
sgs.ai_skill_invoke.hujia = function(self,data)
	...
	for _,p in sgs.list(self.room:getLieges("wei",self.player))do
		if self:isFriend(p) or sgs.ai_role[p:objectName()]=="neutral" then
			return true
		end
	end
end
...
sgs.ai_skill_invoke.fankui = function(self,data)
	...
	if self:isFriend(target) then
		if self:getOverflow(target)>2 then return true end
		return target:hasSkills(sgs.lose_equip_skill) and target:hasEquip()
	end
...
sgs.ai_choicemade_filter.cardChosen.fankui = function(self,player,promptlist)
	...
	if damage.from then
		...
		and self.room:getCardPlace(promptlist[3])==sgs.Player_PlaceEquip
		and damage.from:getEquips():length()>0 then intention = -intention
		elseif sgs.ai_need_damaged.fankui(self,damage.from,player)
		or self:getOverflow(damage.from)>2
		then intention = 0 end
		sgs.updateIntention(player,damage.from,intention)
	end
...
sgs.ai_can_damagehp.fankui = function(self,from,card,to)
...
sgs.ai_need_damaged.fankui = function (self,attacker,player)
	if not player:hasSkills("guicai+fankui") then return false end
	if not attacker then return end
	local need_retrial = function(target)
		local alive_num = self.room:alivePlayerCount()
		return alive_num+target:getSeat() % alive_num>self.room:getCurrent():getSeat()
...
sgs.ai_can_damagehp.ganglie = function(self,from,card,to)
	if from and to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
...
sgs.ai_need_damaged.ganglie = function(self,attacker,player)
	if not attacker then return end
	if not attacker:hasSkill("ganglie") and self:needToLoseHp(attacker,player)
...
sgs.ai_skill_askforyiji.qingjian = function(self,card_ids,tos)
...
sgs.ai_skill_use["@@tuxi"] = function(self,prompt)
	...
	if zhijiangwei and self:isFriend(zhijiangwei) and zhijiangwei:getHandcardNum()==1 and
		...
		for _,enemy in ipairs(self.enemies)do
			if zhijiangwei:canSlash(enemy,slash) and not self:slashProhibit(slash,enemy,zhijiangwei)
			and self:slashIsEffective(slash,enemy,zhijiangwei) and self:isGoodTarget(enemy,self.enemies)
			and self:getDefenseSlash(enemy)<4 then
				isGood = true
			end
		end
	...
	for _,other in sgs.list(others)do
		if self:objectiveLevel(other)>=0 and not (other:hasSkill("tuntian") and other:hasSkill("zaoxian",true)) and add_player(other)==tuxi_mark then
			return parseTuxiCard()
		end
	end
...
sgs.ai_skill_invoke.yiji = function(self)
	if self:needKongcheng(self.player,true)
...
sgs.ai_skill_use["@@yiji"] = function(self,prompt)
	...
	for _,c in sgs.list(self.player:getHandcards())do
		if not extra and c:isKindOf("Jink") or c:isKindOf("Peach")
		or c:isKindOf("Analeptic") then continue end
		table.insert(ids,c:getId())
	end
	for i=1,11 do
		if #ids<1 then break end
		local target,cardid = sgs.ai_skill_askforyiji.nosyiji(self,ids,self.room:getOtherPlayers(self.player))
		if not target or target==self.player then continue end
		if not target1 or target1==target then
			target1 = target
			if #sgs.yijicards1<2 then table.insert(sgs.yijicards1,cardid) end
		elseif not target2 or target2==target then
			target2 = target
			if #sgs.yijicards2<2 then table.insert(sgs.yijicards2,cardid) end
		end
		table.removeOne(ids,cardid)
	end
...
sgs.ai_can_damagehp.yiji = function(self,from,card,to)
...
function SmartAI:shouldUseRende()
	...
	and self:getCardsNum("Slash")>0 then
		...
		for _,enemy in ipairs(self.enemies)do
			if self.player:distanceTo(enemy)==1 or self.player:distanceTo(enemy)==2
			...
			and self:isGoodTarget(enemy,self.enemies) then
				...
				for _,slash in ipairs(self:getCards("Slash"))do
					if not self:slashProhibit(slash,enemy) and self:slashIsEffective(slash,enemy)
					then slash_count = slash_count+1 end
				end
	...
	for _,enemy in ipairs(self.enemies)do
		if enemy:canSlash(self.player) and not self:slashProhibit(nil,self.player,enemy) then
			if enemy:hasWeapon("guding_blade") and self.player:getHandcardNum()==1
			and getCardsNum("Slash",enemy)>=1 then return
			elseif self:hasCrossbowEffect(enemy) and getCardsNum("Slash",enemy)>1
			and self:getOverflow()<=0 then return end
		end
	end
	for _,player in ipairs(self.friends_noself)do
		if player:hasSkill("haoshi") and not player:containsTrick("supply_shortage")
		or player:hasSkill("jijiu") then return true end
	end
	local keepNum = 1
	...
	if self.player:hasSkill("kongcheng") then keepNum = 0 end
	if self:getOverflow()>0 then return true end
	if self.player:getHandcardNum()>keepNum then return true end
	if self.player:getMark("rende")>0 and self.player:getMark("rende")<2
	and (2-self.player:getMark("rende"))>=(self.player:getHandcardNum()-keepNum)
	then return true end
...
sgs.ai_skill_use_func.RendeCard = function(card,use,self)
	...
	for i=1,self.player:getHandcardNum() do
		local h,friend = self:getCardNeedPlayer(cards)
		if h and friend then table.removeOne(cards,h)
		else notFound = true break end
		if friend==self.player
		or not self.player:handCards():contains(h:getId())
		then continue end
		if h:isAvailable(self.player) then
			local canJijiang = friend:getKingdom()=="shu" and self.player:hasLordSkill("jijiang")
			if h:isKindOf("Slash") and not canJijiang or h:isKindOf("Duel") then
				local dummy_use = self:aiUseCard(h)
				if dummy_use.card then
					local t1 = dummy_use.to:first()
					if dummy_use.to:length()>1 or t1:getHp()<2 or getCardsNum("Jink",t1,self.player)<1
					or t1:isCardLimited(dummyCard("jink"),sgs.Card_MethodUse)
					then continue end
				end
			elseif (h:isKindOf("Snatch") or h:isKindOf("Dismantlement"))
			and self:getEnemyNumBySeat(self.player,friend)>0 then
				local dummy_use = self:aiUseCard(h)
				if dummy_use.card then
					local hasDelayedTrick
			...
			elseif h:isKindOf("DelayedTrick")
			and self:getEnemyNumBySeat(self.player,friend)>0
			and self:aiUseCard(h).card
			then continue end
		end
	...
	and self.player:getMark("rende")<2 then
		cards = self:sortByUseValue(self.player:getHandcards(),true)
		for _,p in sgs.list(self.room:getOtherPlayers(self.player))do
			if not self:isEnemy(p) or hasManjuanEffect(p) then
				local to_give = {}
				for _,h in ipairs(cards)do
					if not isCard("Peach",h,self.player)
					then table.insert(to_give,h:getId()) end
...
sgs.ai_skill_use["@@rende"] = function(self,prompt)
	...
	for i=1,self.player:getHandcardNum() do
		...
		local card,friend = self:getCardNeedPlayer(cards)
		if card and friend then table.removeOne(cards,card)
		else return "." end

		if friend==self.player or not self.player:handCards():contains(card:getId()) then continue end
		if card:isAvailable(self.player) then
			local canJijiang = friend:getKingdom()=="shu" and self.player:hasLordSkill("jijiang")
			if card:isKindOf("Indulgence") or card:isKindOf("SupplyShortage") then
				if table.contains(self.toUse,card)
				and self:getEnemyNumBySeat(self.player,friend)>0
				then continue end
			elseif card:isKindOf("Slash") and not canJijiang
			or card:isKindOf("Duel") or card:isKindOf("Snatch")
			or card:isKindOf("Dismantlement") then
				local d = self:aiUseCard(card)
				if d.card and d.to:length()>0 then
					if card:isKindOf("Slash") or card:isKindOf("Duel") then
						local tp = d.to:first()
						if d.to:length()>1 or tp:getHp()<2 or getCardsNum("Jink",tp,self.player)<1
						or tp:isLocked(dummyCard("jink"))
						then continue end
					elseif self:getEnemyNumBySeat(self.player,friend)>0 then
						local hasDelayedTrick
...
sgs.ai_use_revises.rende = function(self,card,use)
	if self.player:getLostHp()>1 and self:findFriendsByType(sgs.Friend_Draw)
	and self.player:usedTimes("RendeCard")<1 then
		if card:getTypeId()==1 
		or card:getTypeId()==2 and not(card:targetFixed() and card:isDamageCard())
		or card:getTypeId()==3 and self:getSameEquip(card)
		then return false end
...
sgs.ai_choicemade_filter.JijiangCard = function(self,player,carduse)
...
sgs.ai_skill_invoke.jijiang = function(self,data)
	...
	for _,p in sgs.list(self.room:getLieges("shu",self.player))do
...
jijiang_skill.getTurnUseCard = function(self)
	for _,p in sgs.list(self.room:getLieges("shu",self.player))do
		if self:isFriend(p) then
			return sgs.Card_Parse("@JijiangCard=.")
		end
	end
end
...
sgs.ai_skill_use_func.JijiangCard = function(card,use,self)
	...
	if self.player:hasFlag("slashTargetFix") then
		for _,p in sgs.list(self.room:getOtherPlayers(self.player))do
			if p:hasFlag("SlashAssignee") then dummy_use.to:append(p) end
		end
	end
	local slash = dummyCard()
...
sgs.ai_skill_cardask["@jijiang-slash"] = function(self,data)
	...
	if not from or not self:isFriend(from) then return "." end
	if self:needBear() then return "." end
	local jijiangtargets = {}
	for _,player in sgs.list(self.room:getAllPlayers())do
		if player:hasFlag("JijiangTarget") then
			if self:isFriend(player) and not self:needToLoseHp(player,from,dummyCard()) then return "." end
	...
	for _,slash in ipairs(self:getCards("Slash"))do
		for _,target in ipairs(jijiangtargets)do
			if not self:slashProhibit(slash,target,from) and self:slashIsEffective(slash,target,from)
...
function sgs.ai_cardsview.jijiang(self,class_name,player)
	local current = global_room:getCurrent()
	if current and current:getKingdom()=="shu" and self:getOverflow(current)>2
	and not self:hasCrossbowEffect(current) then
	...
	for _,p in sgs.list(global_room:getLieges("shu",player))do
		self.player:setFlags("stack_overflow_jijiang")
		local has_friend = self:isFriend(p,player)
		self.player:setFlags("-stack_overflow_jijiang")
...
function SmartAI:getJijiangSlashNum(player)
	...
	if not player:hasLordSkill("jijiang") and not player:hasLordSkill("oljijiang") then return 0 end
	local slashs = 0
...
sgs.ai_view_as.wusheng = function(card,player,card_place)
	if card_place~=sgs.Player_PlaceSpecial and card:isRed() and not card:isKindOf("Peach") then
		return ("slash:wusheng[%s:%s]=%d"):format(card:getSuitString(),card:getNumberString(),card:getEffectiveId())
	end
end
...
wusheng_skill.getTurnUseCard = function(self,inclusive)
	...
	if self.player:hasSkill("nuzhan") then
		for _,enemy in ipairs(self.enemies)do
			if not enemy:hasArmorEffect("EightDiagram")
			and self.player:distanceTo(enemy)<=self.player:getAttackRange()
			and getCardsNum("Jink",enemy)<1
			then nuzhan_equip_e = true break end
		end
		for _,card in ipairs(cards)do
			if card:isRed() and card:isKindOf("TrickCard") and nuzhan_equip_e
			then nuzhan_equip = true break end
		end
	end
	...
	self:sort(self.enemies,"defense")
	if self.player:hasSkill("nuzhan") and not self.player:hasFlag("hasUsedSlash")
	and self:getCardsNum("Slash")>1 then
		for _,enemy in ipairs(self.enemies)do
			if  not enemy:hasArmorEffect("EightDiagram")
			and self.player:distanceTo(enemy)<=self.player:getAttackRange()
			then nuzhan_trick_e = true break end
		end
		for _,card in ipairs(cards)do
			if card:isRed() and card:isKindOf("TrickCard") and nuzhan_trick_e
			then nuzhan_trick = true break end
		end
	end
	...
	if nuzhan_equip then
		for _,card in ipairs(cards)do
			if card:isRed() and card:isKindOf("EquipCard")
			then table.insert(red_card,card) end
		end
	end
	...
	if nuzhan_trick then
		for _,card in ipairs(cards)do
			if card:isRed() and card:isKindOf("TrickCard")
			then table.insert(red_card,card) end
		end
	end
...
yijue_skill.getTurnUseCard = function(self)
	return sgs.Card_Parse("@YijueCard=.")
end
...
sgs.ai_skill_invoke["tishen"] = function(self,data)
	...
	for _,card in ipairs(cards)do
		...
		if card:isKindOf("Slash") then
			for _,enemy in ipairs(self.enemies)do
				if self.player:canSlash(enemy,card,true) and self:isGoodTarget(enemy,self.enemies,card) 
				then
					slashtarget = slashtarget+1
				end
			end
...
sgs.ai_skill_choice.yajiao = function(self,choices,data)
	...
	if self:isFriend(nextAlive) and not self:willSkipDrawPhase(nextAlive) and not self:willSkipPlayPhase(nextAlive)
		...
		for _,skill in ipairs(sgs.getPlayerSkillList(nextAlive))do
			if sgs.ai_cardneed[skill:objectName()] and sgs.ai_cardneed[skill:objectName()](nextAlive,card,self) then return "cancel" end
		end
	end
...
function SmartAI:isValuableCard(card,player)
	...
	if player:getPhase()==sgs.Player_Play then
		if isCard("ExNihilo",card,player) and not player:isLocked(card)
		then return true end
	else
		if isCard("Nullification",card,player) and getCardsNum("Nullification",player,self.player)<2 and player:hasSkills("jizhi|nosjizhi|jilve")
	...
	then return true end
	local id = self:getDangerousCard(player)
	if card:getEffectiveId()==dangerous then return true end
	id = self:getValuableCard(player)
	if card:getEffectiveId()==id then return true end
end
	...
	if (self.player:hasWeapon("Crossbow") or self:getCardsNum("Crossbow")>0) or self:getCardsNum("Slash")>1 then
		...
		for _,enemy in ipairs(self.enemies)do
			if self.player:canSlash(enemy) and self:slashIsEffective(slash,enemy)
				and self:isGoodTarget(enemy,self.enemies,slash) and not self:slashProhibit(slash,enemy) and can_kurou_with_cb(self) then
...
sgs.ai_skill_use["@@lianying"] = function(self,prompt)
	...
	local exclude_self,sn_dis_eff = false,nil
...
sgs.ai_skill_use_func.GuoseCard = function(card,use,self)
	...
	if not indul_only and not self.player:isJilei(rcard)
	then
		...
		for _,friend in ipairs(self.friends)do
			...
			then
				for _,c in sgs.list(friend:getJudgingArea())do
					...
					and self.player:canDiscard(friend,card:getEffectiveId())
					then
						use.card = card
						use.to:append(friend)
						return
	...
	if not indul_only and not self.player:isJilei(rcard) then
		for _,friend in ipairs(self.friends)do
			...
			then
				for _,c in sgs.list(friend:getJudgingArea())do
					if c:isKindOf("Indulgence") and self.player:canDiscard(friend,card:getEffectiveId())
					then
						use.card = card
						use.to:append(friend)
						return
...
jieyin_skill.getTurnUseCard=function(self)
	...
	return sgs.Card_Parse(("@JieyinCard=%d+%d"):format(first,second))
end


function SmartAI:getWoundedFriend(maleOnly,include_self,players)
	local friends = {}
	if not players then
		...
		if not include_self and table.contains(friends,self.player) then table.removeOne(friends,self.player) end
	end
	self:sort(friends,"hp")
	...
	for _,friend in ipairs(friends)do
		if friend:isLord() then
			...
			and self:getEnemyNumBySeat(self.player,friend)<=(friend:getHp()>=2 and 1 or 0)
			then addToList(friend,2)
			elseif self:needToLoseHp(friend,nil,nil,true,true)
			then addToList(friend,2)
			elseif not sgs.isLordHealthy()
			then addToList(friend,1) end
		else
	...
	local cmp = function (a,b)
		local cha = getCmpHp(a)
		local chb = getCmpHp(b)
		if cha==chb then
...
sgs.ai_card_intention.ChuliCard = function(self,card,from,tos)
	for _,to in ipairs(tos)do
		if self.chuli_id_choice and self.chuli_id_choice[to:objectName()] then
			local em_prompt = {"cardChosen","chuli",tostring(self.chuli_id_choice[to:objectName()]),to:objectName()}
...
sgs.ai_view_as.jijiu = function(card,player,card_place)
	if card_place~=sgs.Player_PlaceSpecial and card:isRed() then
		return ("peach:jijiu[%s:%s]=%d"):format(card:getSuitString(),card:getNumberString(),card:getEffectiveId())
	end
end
...
local jijiu = {}
jijiu.name = "jijiu"
table.insert(sgs.ai_skills,jijiu)
jijiu.getTurnUseCard = function(self)
	local cards = self.player:getCards("he")
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards,true)
	for _,c in ipairs(cards)do
		if c:isRed() then
			return sgs.Card_Parse("peach:jijiu[no_suit:0]="..c:getEffectiveId())
		end
	end
end
...
sgs.ai_skill_use["@multi-jink-start"] = function(self,prompt)
	local data = self.room:getTag("SlashData")
	local pr = prompt:split(":")
	return sgs.ai_skill_cardask["@multi-jink-start"](self,data,"jink",data:toCardEffect().from,nil,pr[#pr])
end

sgs.ai_skill_use["@multi-jink"] = sgs.ai_skill_use["@multi-jink-start"]
...
local function getPriorFriendsOfLiyu(self,lvbu)
	...
	if not string.startsWith(self.room:getMode(),"06_") and not sgs.GetConfig("EnableHegemony",false) then
		if lvbu:isLord() then
			for _,friend in ipairs(self:getFriends(lvbu,true))do
				if sgs.ai_role[friend:objectName()]=="loyalist" then table.insert(prior_friends,friend) end
			end
	...
	elseif self.room:getMode()=="06_XMode" then
		...
		if leader then
			...
			if backup==0 then
				prior_friends = self:getFriends(lvbu,true)
			end
		end
	end
...
function SmartAI:findLijianTarget(card_name,use)
	...
	if lord and self:isFriend(lord) and lord:hasSkill("hunzi") and lord:getHp()==2 and lord:getMark("hunzi")==0 and lord:objectName()~=self.player:objectName() then
		...
		if peaches>=enemycount then
			...
			for _,ap in sgs.list(self.room:getOtherPlayers(self.player))do
	...
	if shenguanyu and shenguanyu:isMale() and shenguanyu:objectName()~=self.player:objectName() then
		if self.role=="rebel" and lord and lord:isMale() and lord:objectName()~=self.player:objectName() and shenguanyu:objectName()~=lord:objectName()
...
sgs.ai_choicemade_filter.LijianCard = function(self,player,carduse)
	sgs.ai_lijian_effect = true
end

sgs.ai_card_intention.LijianCard = function(self,card,from,to)
	if sgs.ai_role[to[1]:objectName()]==sgs.ai_role[to[2]:objectName()] then
		if sgs.ai_role[from:objectName()]=="rebel" and sgs.ai_role[to[1]:objectName()]==sgs.ai_role[from:objectName()] and to[1]:getHp()==1 then
		elseif to[1]:hasSkill("hunzi") and to[1]:getHp()==2 and to[1]:getMark("hunzi")==0 then
		else
			sgs.updateIntentions(from,to,40)
		end
	elseif sgs.ai_role[to[1]:objectName()]~=sgs.ai_role[to[2]:objectName()] and not to[1]:hasSkill("wuhun") then
...
sgs.ai_skill_invoke.qiaomeng = function(self,data)
	...
	return self:doDisCard(damage.to,"e")
end

function sgs.ai_skill_invoke.wangxi(self,data)
	local target = data:toPlayer()
	if target then
		if self:isFriend(target) and not self:needKongcheng(target)
		then return true
		else
...
sgs.ai_choicemade_filter.skillInvoke.wangxi = function(self,player,promptlist)
	local damage = self.room:getTag("CurrentDamageStruct"):toDamage()
	local target = nil
	if damage.from==player then target = damage.to
	elseif damage.to==player then target = damage.from end
	if target and promptlist[3]=="yes" then
		if self:needKongcheng(target,true) then sgs.updateIntention(player,target,10)
		elseif not hasManjuanEffect(target) and player:getState()=="robot"
		then sgs.updateIntention(player,target,-60) end
	end
end

sgs.ai_can_damagehp.wangxi = function(self,from,card,to)
	if from and to:getHp()+self:getAllPeachNum()-self:ajustDamage(from,to,1,card)>0
	and self:canLoseHp(from,card,to) then return self:isFriend(from) end
end
...
sgs.ai_skill_cardask["@zhuhai-slash"] = function(self,data,pattern,target,target2)
	for _,slash in ipairs(self:getCards("Slash"))do
		...
		and not self:needToLoseHp(target2,self.player,slash) and not self:findLeijiTarget(target2,50,self.player)
		then
...
jianyan_skill.getTurnUseCard = function(self)
	return sgs.Card_Parse("@JianyanCard=.")
end
