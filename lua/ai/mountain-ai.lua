function SmartAI:card_for_qiaobian(who,flag,to_friends,to_enemies)
	...
	if self:isFriend(who) then
			...
			for _,judge in sgs.qlist(who:getJudgingArea())do
				if not judge:isKindOf("YanxiaoCard") then
					for _,enemy in ipairs(to_enemies)do
						if not enemy:containsTrick("YanxiaoCard")
						and not self.player:isProhibited(enemy,judge)
						and not (enemy:hasSkills("hongyan|olhongyan") or judge:isKindOf("Lightning"))
						then return enemy,judge end
					end
				end
			end
		end
		if flag:match("e") and who:hasEquip() then
			if who:hasSkills(sgs.lose_equip_skill) then
				for _,equip in sgs.qlist(who:getCards("e"))do
					if self:doDisCard(who,equip:getEffectiveId(),true) then
				...
				if card then
					...
					if #self:poisonCards({card},who)>0 then
						self:sort(to_enemies,"defense")
						for _,friend in ipairs(to_enemies)do
						...
						or card:isKindOf("DefensiveHorse") then
						...
						for _,friend in ipairs(to_friends)do
							if not friend:hasEquipArea(index) then continue end
							if not self:getSameEquip(card,friend) and friend:objectName()~=who:objectName()
							and friend:hasSkills(sgs.need_equip_skill.."|"..sgs.lose_equip_skill)
							then return friend,card end
						end
						for _,friend in ipairs(to_friends)do
							if not friend:hasEquipArea(index) then continue end
		...
		if flag:match("j")
		and who:containsTrick("YanxiaoCard") then
			for _,judge in sgs.qlist(who:getJudgingArea())do
		...
		if flag:match("e") and who:hasEquip() then
			if who:hasSkills(sgs.lose_equip_skill) then return end
			...
			if card and #self:poisonCards({card},who)<1 then
				local index = card:getRealCard():toEquipCard():location()
				if card:isKindOf("Armor")
				or card:isKindOf("DefensiveHorse") then
					self:sort(to_friends,"defense")
				...
				for _,friend in ipairs(to_friends)do
					if not friend:hasEquipArea(index) then continue end
					if not self:getSameEquip(card,friend) and friend:objectName()~=who:objectName()
					and friend:hasSkills(sgs.lose_equip_skill.."|shensu|tenyearshensu")
					then return friend,card end
				end
				for _,friend in ipairs(to_friends)do
					if not friend:hasEquipArea(index) then continue end
...
sgs.ai_slash_prohibit.tuntian = function(self,from,to,card)
	...
	if getCardsNum("Jink",to,from)<1 or self:isWeak(to) then return false end
...
sgs.ai_skill_use_func.ZhijianCard = function(card,use,self)
	...
	use.card = sgs.Card_Parse("@ZhijianCard="..select_equip:getId())
end
...
sgs.ai_suit_priority.jiang = function(self,card)
	return (card:isKindOf("Slash") or card:isKindOf("Duel")) and "diamond|heart|club|spade"
end
...
function SmartAI:needBear(player)
    ...
    return player:hasSkills("renjie+baiyin") and not player:hasSkill("jilve",true) and player:getMark("&bear")<4
end
...
sgs.ai_skill_choice.mobilelianpo = function(self,choices)
	choices = choices:split("+")
	return choices[1]
end

sgs.ai_skill_invoke.mobilejilve = function(self,data)
	return self:canDraw()
end

sgs.ai_skill_choice.mobilejilve = function(self,choices)
	choices = choices:split("+")
	if self:getOverflow()<2 or self:isWeak() then
		return choices[2]
	end
