...
### Android 直向重疊修正（2026-09-21，來源檢查點）

- Dashboard 改為共同計算操作列、技能列與手牌列；最低高度包含第二排手牌上移及選牌抬升，技能列依原生 dock 高度預留，取消固定的 108 座標。
- 使用者截圖另顯示自身操作區偏右下並遭裁切：直向不再因可用寬度達 700 而切回橫向 Dashboard，自適應排版亦清除原橫向底部中央縮放原點，避免縮放後位移。
- 沿用原生牌、按鈕、角色與選取狀態；裝備覆蓋角色仍是既有設計。
- 新增手機寬度、單／雙將及多排技能的幾何回歸案例；focused 尚未執行。
- 第二輪人工截圖確認標記區也壓到托管：Dashboard 在標記／階段更新後重排狀態項目至工具按鈕及倒數條上方，寬度不足時將階段標籤另起一列；橫向與 Photo 維持皮膚錨點。第二輪 Android 建置、保留資料覆蓋安裝與啟動成功，證據另存 `builds/android-portrait-marks-20260921/`，標記重現及 GUI 互動尚待驗收。
- 第一輪 Android x86_64 Debug APK 建置成功，以 `adb install -r` 保留資料更新目前 LDPlayer 並啟動。直向對局截圖已顯示底部操作區、手牌與自身角色回到畫面內；該輪階段標籤仍壓到取消按鈕上緣，已由上述第二輪補修，尚待確認。證據：`builds/android-portrait-fix-20260921/`。非串流安裝失敗後改串流安裝成功；沒有清除資料，沒有宣告完整對局通過。
