...
召喚佇列狀態已移入 [`src/server/player-lifecycle-service.h`](../src/server/player-lifecycle-service.h)（`PlayerLifecycleService`）：
```cpp
...
`Room` 層只剩轉發實作（[`src/server/room.cpp`](../src/server/room.cpp)）：
```cpp
...
主要方法實現（[`src/server/player-lifecycle-service.cpp`](../src/server/player-lifecycle-service.cpp)）：
...
處理點已封裝進 `RoomThread::flushOutermostDeferredWork()`（[`src/server/roomthread.cpp`](../src/server/roomthread.cpp)），在事件堆疊清空時呼叫：
```cpp
