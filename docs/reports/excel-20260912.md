# Excel 前端實作與驗證 — 2026-09-12

結果對應 2026-09-12 L 工作樹及下列證據目錄的來源快照。封裝操作見[Excel 封裝指南](../excel-packaging.md)。
...
| CP2 可玩牌桌 | 五工作表 VBA 來源、範圍內 28 類操作路徑及非同步 transport 已接線；QML 排除 | 實際 `.xlsm`、VBE 編譯、Excel 操作未完成 |
...
QML 不實作於 `.xlsm` 版。現代與舊版 Excel 套件均排除 `qml_interact`／`qsanguosha.qml`，不再以缺少 QML presenter 阻擋完整交付驗收。標準互動驗收範圍因此為 28 類；原生登錄仍有 29 類。
...
現代 `qsanguosha_excel_bridge`／`qsanguosha_excel_server`、舊版 `qsanguosha_excel_bridge`／`QSanguoshaXPServer`，以及三個現代 `qsanguosha_excel_*_tests` 已完成 Debug 建置。三組 focused tests 各設 60 秒外部上限；不包含本地 CTest、完整對局或矩陣。
