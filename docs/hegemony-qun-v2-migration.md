...
`lang/zh_CN/Package/HStandardQunGeneral.lua` 同步移除 51 個已退役或重複的翻譯鍵；保留 15 名武將的名稱／稱號、8 個獨立技能及其提示，並補齊雙刃的選殺目標與距離 helper 名稱。共用技能直接讀取原翻譯，狂斧裝備選項沿用 `HegemonyPackage.lua`。技能正文維持簡體中文，保留原始來源註記與武將名稱後的 `[国]`。
