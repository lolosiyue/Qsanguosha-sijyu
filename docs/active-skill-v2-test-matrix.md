...
| Lua factory／enum smoke 與 Room 初始化 | `lua/test/examples/test_active_skill_v2_usage_identity.lua` | 待執行；`QSanguosha.exe --lua-test lua/test/examples/test_active_skill_v2_usage_identity.lua`，assertion 失敗必須回傳非零 |
...
`Usage_SourceInstance` 計數、由 server 建立普通 Slash 的 C++ 合成技能；
...
`lua/test/examples/test_active_skill_v2_usage_identity.lua` 是等價 Lua fixture：它以
`sgs.CreateActiveSkillV2` 與 `sgs.CreateTriggerV2Skill` 驗證兩個 usage identity enum、
factory 設值及無效 enum fail-closed，且不會將測試武將包載入一般對局。
執行入口為：
`QSanguosha.exe --lua-test lua/test/examples/test_active_skill_v2_usage_identity.lua`。
...
- Ticket 13 已完成核心與測試 fixture，但尚未執行工具鏈：`usage_identity` 已把時間範圍 `limit_scope` 與配額身份拆開；source identity、nested reservation、pay/cancel release、bypass commit、reset、Custom 邊界及控制事件收束均已有代碼／案例。在 Release x64 與 Room lifecycle 實測前，不得宣稱整合測試通過。
