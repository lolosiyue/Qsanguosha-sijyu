...
- 2026-08-30 已完成 Protocol V1 codec boundary、V1-compatible capability
  negotiation、codec-neutral `ProtocolMessage` 與 Protocol V2 envelope／codec
  contract。`Packet` 保留 compatibility facade，production socket、active protocol、
  framing、command IDs 與 replay stream 仍為 V1；runtime activation 尚未開始。
...
| M4 | In Progress（V2 codec contract complete；runtime activation pending） | 協定與重播版本化、相容性拒絕路徑 | 新舊版本差異可診斷；不支援版本被明確拒絕而非靜默誤讀 |
