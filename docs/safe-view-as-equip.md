...
> **更正（2026-08-21 校對）**：[`lua/sgs_ex.lua`](../lua/sgs_ex.lua) 並未提供 `sgs.SafeTurn*` 通用 Lua API；安全換裝的權威實作為 [`src/package/yjcm2023.cpp`](../src/package/yjcm2023.cpp) 內的**靜態 C++ helper** `zhizheEquipObjectNameByArea()`／`safeTurnCardToEquip()`，僅由 `GongqiaoCard::use` 等 C++ 技能直接調用。Lua 擴充如需同等安全路徑，應在各自技能內參照該 helper 的 `cloneCard`→`WrappedCard::takeOver`→換裝流程自行實作，或將 helper 抽為共用工具後再以 SWIG 暴露（尚未落地）。
