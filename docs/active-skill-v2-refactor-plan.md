# ViewAsSkillV2 與 SkillCard／ViewAsSkill 契約

核心類型與 Lua 暴露入口見 [`swig/sanguosha.i`](../swig/sanguosha.i) 的 `SkillInstanceRef`／`ActiveSkillRequest`。實作歷程與版本驗證由文末連結進入。

## 1. 文件定位

本文件是 `ViewAsSkillV2`、舊 `SkillCard/ViewAsSkill` 過渡橋接及相關多實例整合的設計契約。
