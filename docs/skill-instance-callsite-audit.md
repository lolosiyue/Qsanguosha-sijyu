...
> **快照存檔（2026-07-16）**：本文件的計數（`acquireSkill(` 660、當時 `Room::acquireSkill` 156）為 2026-07-16 lexical inventory，會隨後續提交漂移（2026-08-21 全倉含 `extensions/` 粗略統計已約 1,500 處）。**正文計數不得當作現況**；當前狀態請以 `rg` 重跑為準（見末節命令）。本文件保留作優先級分類與「ensure／stack／replace／temporary」審核規則參考。
