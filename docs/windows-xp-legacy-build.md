...
The 2026-09-20 snapshot-memory checkpoint built both Release executables with
the installed Qt 5.6.3 tree and v141_xp. Both passed the canonical x86 / PE
5.01 and import checks; the server passed its module-import boundary check.
History bindings were regenerated by SWIG and compiled. Evidence is in
`builds/win32-snapshot-memory-20260920/xp-release-rebuild-3.log`.
This checkpoint does not establish XP guest execution, full-game acceptance
or a measured RAM reduction for the snapshot-memory changes.
