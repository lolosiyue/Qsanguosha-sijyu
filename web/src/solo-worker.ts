...
function result(): Record<string, unknown> {
  if (!runtime) throw new Error("单机执行环境尚未就绪");
...
async function download(path: string, limit: number): Promise<Uint8Array> {
  ...
  if (!response.ok || response.redirected) throw new Error(`游戏包缺少档案：${path}`);
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (bytes.length > limit) throw new Error(`游戏档案超过大小限制：${path}`);
  return bytes;
}
async function prepare(): Promise<void> {
  if (runtime || disposed || failed) throw new Error("请重新建立单机执行环境");
  ...
  if (manifest?.schema_version !== 1) throw new Error("单机发布版本不支援");
  ...
  for (const [entry, bytes] of [[moduleEntry, moduleBytes], [wasmEntry, wasmBinary]] as const) {
    if (bytes.length !== entry.size || await sha256(bytes) !== entry.sha256)
      throw new Error("单机执行档版本不一致，请重新解压完整游戏包");
  }
  ...
  options.preInit = [() => {
    if (!options.ENV) throw new Error("单机执行环境缺少 ENV");
  ...
  options.preRun = [(module?: SoloModule) => {
    ...
    if (!fs) throw new Error("单机执行环境缺少档案系统");
  ...
  for (const name of ["_qsan_solo_initialize", "_qsan_solo_start", "_qsan_solo_frame", "_qsan_solo_pump", "_qsan_solo_stop"] as const)
    if (typeof runtime[name] !== "function") throw new Error("单机执行档缺少必要入口");
...
async function receive(message: Record<string, unknown>): Promise<void> {
  ...
  if (!runtime || failed || disposed) throw new Error("单机执行环境尚未就绪");
  if (message.type === "start") {
    if (started) throw new Error("一个执行环境只可建立一局");
    started = true;
