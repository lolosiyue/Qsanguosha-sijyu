...
function result(): Record<string, unknown> {
  if (!runtime) throw new Error("单机运行环境尚未就绪");
  const bytes = runtime.FS.readFile("/work/solo-result.json");
  if (bytes.length > 8 * 1024 * 1024) throw new Error("单机回复超过大小限制");
  const value: unknown = JSON.parse(decoder.decode(bytes));
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("单机回复格式错误");
  return value as Record<string, unknown>;
}
function check(status: number): Record<string, unknown> {
  const value = result();
  if (status !== 0 || value.phase === "failed") throw new Error(String(value.error || value.reason || "单机执行失败"));
  return value;
}
async function download(path: string, limit: number): Promise<Uint8Array> {
  ...
  if (!response.ok || response.redirected) throw new Error(`游戏包缺少文件：${path}`);
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (bytes.length > limit) throw new Error(`游戏文件超过大小限制：${path}`);
  return bytes;
}
async function prepare(): Promise<void> {
  if (runtime || disposed || failed) throw new Error("请重新建立单机运行环境");
  if (!globalThis.crossOriginIsolated || typeof SharedArrayBuffer === "undefined")
    throw new Error("请使用完整游戏包内的 StartGame.exe 启动游戏");
  const manifest = JSON.parse(decoder.decode(await download("/solo/manifest.json", 4 * 1024 * 1024)));
  if (manifest?.schema_version !== 1) throw new Error("单机发布版本不支持");
  ...
  for (const [entry, bytes] of [[moduleEntry, moduleBytes], [wasmEntry, wasmBinary]] as const) {
    if (bytes.length !== entry.size || await sha256(bytes) !== entry.sha256)
      throw new Error("单机可执行文件版本不一致，请重新解压完整游戏包");
  }
  ...
  const options: Record<string, unknown> & { ENV?: Record<string, string>; FS?: SoloFs } = {
    ...
    locateFile: (path: string) => {
      ...
      throw new Error(`未知的单机可执行文件：${path}`);
    },
    print: (line: unknown) => console.info(String(line)),
    printErr: (line: unknown) => console.warn(String(line)),
    onAbort: (reason: unknown) => report(new Error(`单机执行中止：${String(reason)}`)),
  };
  options.preInit = [() => {
    if (!options.ENV) throw new Error("单机运行环境缺少 ENV");
  ...
  options.preRun = [(module?: SoloModule) => {
    ...
    if (!fs) throw new Error("单机运行环境缺少文件系统");
  ...
  for (const name of ["_qsan_solo_initialize", "_qsan_solo_start", "_qsan_solo_frame", "_qsan_solo_pump", "_qsan_solo_stop"] as const)
    if (typeof runtime[name] !== "function") throw new Error("单机可执行文件缺少必要入口");
...
async function receive(message: Record<string, unknown>): Promise<void> {
  ...
  if (!runtime || failed || disposed) throw new Error("单机运行环境尚未就绪");
  if (message.type === "start") {
    if (started) throw new Error("一个运行环境只可建立一局");
    started = true;
    ...
    if (!started || typeof message.frame !== "string") throw new Error("单机输入格式错误");
    const bytes = encoder.encode(message.frame);
    if (bytes.length > 4 * 1024 * 1024) throw new Error("单机输入超过大小限制");
