...
function seed(): void {
  ...
  for (let i = 0; i < playerCount; ++i) {
    ...
    Object.assign(session.state.ensurePlayer(name), {
      screen_name: i ? `${tr(generals[i])} · ${i + 1}号位` : "你的座位",
  ...
  session.state.appendPresentationEvent(Command.SPEAK, "", { player_name: "player-2", text: "这里是房内聊天区。" });
  ui.selectedCards = [];
  ui.selectedPlayers = [];
  ui.selectedOption = "";
  ...
  session.interaction = scene === "lobby" ? null : {
    ...
    payload: scene === "generals"
      ? { options: generals.slice(0, 5), prompt: "选择武将，再按确认。" }
      : { players: session.state.playerNames.filter((name) => name !== "self"), prompt: "选择一名角色，预览目标选取效果。" }
  };
}
...
function render(): void {
  ...
  const count = el("select", { "aria-label": "预览人数" });
  ...
  toolbar.append(scenes, count, el("a", { href: "/" }, ["返回首页"]));
