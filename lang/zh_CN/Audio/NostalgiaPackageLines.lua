-- lines for Nostalgia Package

return {
	--黄月英
	["$nosjizhi1"] = "哼。",
	["$nosjizhi2"] = "哼哼。",
	["~nos_huangyueying"] = "亮。",
	--貂蝉
	["$noslijian1"] = "嗯呵呵~呵呵~",
	["$noslijian2"] = "夫君，你要替妾身做主啊！",
	["$biyue1"] = "失礼啦~",
	["$biyue2"] = "羡慕吧~",

	["$lijian1"] = "这场比赛，将军可要赢哦~",
	["$lijian2"] = "赢家，才能得到我。",
	["$biyue3"] = "水映月明，芙蓉照倩影。",
	["$biyue4"] = "梦蝶幻月，如沫虚妄。",

	["~nos_diaochan"] = "父亲大人，对不起……",
	["~diaochan"] = "我的任务，终于完成了……",

	--刘备

	["$nosrende1"] = "惟贤惟德，能服于人。",
	["$nosrende2"] = "以德服人。",
	-- 旧 曹操
	["$nosjianxiong1"] = "宁教我负天下人，休教天下人负我！",
	["$nosjianxiong2"] = "吾好梦中杀人！",
	["$hujia3"] = "魏将何在？",
	["$hujia4"] = "来人，护驾！",
	["~nos_caocao"] = "霸业未成，未成啊……",

	-- 旧 司马懿
	["$nosguicai1"] = "天命？哈哈哈哈……",
	["$nosguicai2"] = "吾乃天命之子！",
	["$nosfankui1"] = "下次注意点。",
	["$nosfankui2"] = "出来混，早晚要还的。",
	["~nos_simayi"] = "难道真是天命难违？",


	-- 夏侯惇
	["$nosganglie1"] = "鼠辈，竟敢伤我！",
	["$nosganglie2"] = "以彼之道，还施彼身！",
	["~nos_xiahoudun"] = "两边都看不见了……",

	-- 旧 张辽
	["$nostuxi1"] = "哼，没想到吧！",
	["$nostuxi2"] = "拿来吧！",
	["~nos_zhangliao"] = "真的没想到……",

	-- 旧 许褚
	["$nosluoyi1"] = "脱！",
	["$nosluoyi2"] = "谁来与我大战三百回合？",
	["~nos_xuchu"] = "冷，好冷啊……",

	-- 郭嘉
	["$tiandu3"] = "就这样吧。",
	["$tiandu4"] = "哦？",
	["$nosyiji1"] = "也好。",
	["$nosyiji2"] = "罢了。",
	["~nos_guojia"] = "咳，咳……",

	-- 旧 关羽
	["$wusheng3"] = "关羽在此，尔等受死！",
	["$wusheng4"] = "看尔乃插标卖首！",
	["~nos_guanyu"] = "什么？此地名叫麦城？",

	-- 旧 张飞
	["$paoxiao3"] = "啊~~~",
	["$paoxiao4"] = "燕人张飞在此！",
	["~nos_zhangfei"] = "实在是杀不动了……",

	-- 旧 赵云
	["$longdan3"] = "能进能退，乃真正法器！",
	["$longdan4"] = "吾乃常山赵子龙也！",
	["~nos_zhaoyun"] = "这，就是失败的滋味吗？",

	-- 旧 马超
	["$nostieji1"] = "全军突击！",
	["$nostieji2"] = "（蹄声、马叫声）",
	["~nos_machao"] = "（马蹄声……）",

	-- 旧 甘宁
	["$qixi3"] = "接招吧！",
	["$qixi4"] = "你的牌太多了！",
	["~nos_ganning"] = "二十年后，又是一条好汉！",

	-- 旧 吕蒙
	["$keji3"] = "不是不报，时候未到！",
	["$keji4"] = "留得青山在，不怕没柴烧！",
	["~nos_lvmeng"] = "被看穿了吗？",

	-- 旧 黄盖
	["$noskurou1"] = "请鞭笞我吧 ，公瑾！",
	["$noskurou2"] = "赴汤蹈火，在所不辞！",
	["~nos_huanggai"] = "失血过多了……",

	-- 旧 周瑜
	["$nosyingzi1"] = "哈哈哈哈……",
	["$nosyingzi2"] = "汝等看好了！",
	["$nosfanjian1"] = "挣扎吧，在血和暗的深渊里！",
	["$nosfanjian2"] = "痛苦吧，在仇与恨的地狱中！",
	["~nos_zhouyu"] = "既生瑜，何生……",

	-- 旧 大乔
	["$nosguose1"] = "请休息吧。",
	["$nosguose2"] = "你累了。",
	["$liuli3"] = "交给你了。",
	["$liuli4"] = "你来嘛~",
	["~nos_daqiao"] = "伯符，我去了……",

	-- 陆逊
	["$nosqianxun1"] = "儒生脱尘，不为贪逸淫乐之事。",
	["$nosqianxun2"] = "谦谦君子，不饮盗泉之水。",
	["$noslianying1"] = "牌不是万能的，但是没牌是万万不能的。",
	["$noslianying2"] = "旧的不去，新的不来。",
	["~nos_luxun"] = "我还是太年轻了……",

	-- 旧 华佗
	["$jijiu3"] = "别紧张，有老夫呢。",
	["$jijiu4"] = "救人一命，胜造七级浮屠。",
	["$qingnang1"] = "早睡早起，方能养生。",
	["$qingnang2"] = "越老越要补啊。",
	["~nos_huatuo"] = "医者…不能自医啊……",

	-- 旧 吕布
	["$wushuang3"] = "谁能挡我！",
	["$wushuang4"] = "神挡杀神，佛挡杀佛！",
	["~nos_lvbu"] = "不可能！",

	-- 旧 曹仁
	["$nosjushou1"] = "我先休息一会儿！",
	["$nosjushou2"] = "尽管来吧！",
	["~nos_caoren"] = "实在是守不住了……",

	-- 旧 周泰
	["$nosbuqu1"] = "我绝不会倒下！",
	["$nosbuqu2"] = "还不够！",
	["~nos_zhoutai"] = "已经尽力了……",

	-- 旧 张角
	["$nosleiji1"] = "雷公助我！",
	["$nosleiji2"] = "以我之真气，合天地之造化。",
	["$guidao3"] = "哼哼哼哼……",
	["$guidao4"] = "天下大势，为我所控！",
	["$huangtian3"] = "苍天已死，黄天当立！",
	["$huangtian4"] = "岁在甲子，天下大吉！",
	["~nos_zhangjiao"] = "黄天…也死了……",

	-- 旧 于吉
	["$nosguhuo1"] = "你信吗？",
	["$nosguhuo2"] = "猜猜看哪？",
	["~nos_yuji"] = "竟然…被猜到了……",

	-- 旧 法正
	["cv:nos_fazheng"] = "苏遥东，官方",
	["$nosenyuan1"] = "得人恩果千年记。",
	["$nosenyuan2"] = "滴水之恩，涌泉以报。",
	["$nosenyuan3"] = "谁敢得罪我！",
	["$nosenyuan4"] = "睚眦之怨，无不报复。",
	["$nosxuanhuo1"] = "重用许靖，以眩远近。",
	["$nosxuanhuo2"] = "给你的，十倍奉还给我！",
	["~nos_fazheng"] = "辅翼既折，蜀汉衰矣……",

	-- 旧 凌统
	["cv:nos_lingtong"] = "官方",
	["$nosxuanfeng1"] = "伤敌于千里之外！",
	["$nosxuanfeng2"] = "索命于须臾之间！",
	["~lingtong"] = "大丈夫...不惧死亡…",

	-- 旧 徐庶
	["$noswuyan1"] = "唉，一切尽在不言中。",
	["$noswuyan2"] = "嘘，言多必失啊。",
	["$nosjujian1"] = "我看好你！",
	["$nosjujian2"] = "将军岂愿抓牌乎？",
	["~nos_xushu"] = "娘，孩儿不孝，向您…请罪……",

	-- 旧 关兴张苞
	["cv:nos_guanxingzhangbao"] = "官方",
	["$nosfuhun1"] = "不血父仇，誓不罢休！",
	["$nosfuhun2"] = "承父遗志，横扫叛贼！",
	["$nosfuhun3"] = "父辈功勋，望尘莫及……",
	["$wusheng10"] = "蜀汉重任，后继有人！", 
	["$paoxiao9"] = "哼！", 
	["~nos_guanxingzhangbao"] = "吾得父亲之遗志，未袭父亲之神勇。",

	-- 旧 韩当
	--["cv:nos_handang"] = "风叹息",
	["cv:nos_handang"] = "官方",
	--[[["$nosgongqi1"] = "弓马齐备，远射近突！",
	["$nosgongqi2"] = "弓马齐备，远射近突！",
	["$nosjiefan1"] = "解烦军至，此危自解！",
	["$nosjiefan2"] = "吾主莫慌，韩义公在此！",
	["~nos_handang"] = "我主堪忧，我主堪忧啊……",]]
	["$nosgongqi1"] = "鼠辈，哪里走！",
	["$nosgongqi2"] = "吃我一箭！",
	["$nosjiefan1"] = "休想乘人之危！",
	["$nosjiefan2"] = "退后，这里交给我！",
	["~nos_handang"] = "今后，只能靠你了……",

	-- 旧 马岱
	--["cv:nos_madai"] = "风叹息",
	["cv:nos_madai"] = "官方",
	["$nosqianxi2"] = "斩草除根，除恶务尽！",
	["$nosqianxi1"] = "伤其十指，不如断其一指！",
	["$nosqianxi3"] = "竟然有防备？只能力战搏杀之！", -- 判定失败

	-- 旧 王异
	["cv:nos_wangyi"] = "蒲小猫",
	["$noszhenlie1"] = "雪君父之大耻，虽丧身亦不惜。", -- 回合外
	["$noszhenlie2"] = "顾子而不行，不如先死矣。", -- 回合内
	["$nosmiji1"] = "有此九奇，可逐羌敌。",
	["$nosmiji2"] = "屯兵逐超，得保冀城。", -- 自己
	["$nosmiji3"] = "救兵已到，乃解城围。", -- 他人
	["$nosmiji4"] = "暂与汝和，仇必后报！", -- 马超

	-- 旧 刘表
	--["cv:nos_liubiao"] = "喵小林",
	["cv:nos_liubiao"] = "官方",
	["$noszishou1"] = "荆襄之地，固若金汤。",
	["$noszishou2"] = "江河霸主，何惧之有？",
	--["$noszishou3"] = "余粮甚厚，不急出兵。",
	--["$noszishou2"] = "按兵不动，徐图荆襄霸业。",
	--["$noszishou3"] = "忍时待机，以期坐收渔利。",
	["~nos_liubiao"] = "不欲争天下，奈何……",

	-- 旧 朱桓
	["$youdi1"] = "无名小卒，可敢再前进一步？",
	["$youdi2"] = "予以小利，必有大获",
	["~nos_zhuhuan"] = "这巍巍巨城，吾竟无力撼动" ,

	-- 旧 曹叡
	["$nosmingjian1"] = "你我推心置腹，岂能相负？",
	["$nosmingjian2"] = "孰忠孰奸，朕尚能明辨！",
	["~nos_caorui"] = "悔不该耽于逸乐，致有今日……" ,

	-- 旧 曹休
	["$taoxi1"] = "策马疾如电，溃敌一瞬间！",
	["$taoxi2"] = "虎豹骑岂能徒有虚名！杀！！",
	["~nos_caoxiu"] = "兵行险招，终有一失……" ,
}