...
int main(int argc, char *argv[]) {
    ...
    // A mistyped profile name must not be silently treated as unspecified — CI would believe
    // it ran none while actually running full.
    {
    ...
    // package/dev tree) and setCurrent()s it so launching from any CWD works. Relative
    // report/log arguments are therefore relative to the asset root; the runner always
    ...
    if (appArgs.contains(QStringLiteral("--asset-report"))) {
        ...
        // --asset-manifest lets an uninstalled build tree (dev machines and CI) answer "which
        // assets are expected to be missing"; the manifest is generated by CMake into the
        // build directory.
    ...
    // startup smoke: QApplication already exists; take over the Qt message hook here and
    // register the application stage. This entry point never returns before QApplication
    ...
    // Translations: the install tree keeps them in share/qsanguosha/translations/, legacy
    // deployments in the asset root. Both are tried, so the layout change never silently
    ...
    if (uiStartupSmoke) {
        // From here on, follow the same path as a normal startup: create the real MainWindow,
        // load the real HomeScene, run the real Qt event loop, and exit automatically once
        // the ready condition is met.
        const int rc = UiStartupSmokeController::run();
        exitStartupSmoke(rc);
    }

    if (multimediaSmoke) {
        // Same as above, then once HomeScene is ready run the audio backend / voice pool /
        // BGM / video background stage and finally tear the media resources down cleanly.
    ...
    if (effectsSmoke) {
        // Same as above, then once HomeScene is ready run the effects policy / completion
        // contract, the missing-asset degradation of GIF/Spine/frame animation, and the
        // profile object budget.
