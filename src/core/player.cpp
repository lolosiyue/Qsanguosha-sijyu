...
bool Player::inMyAttackRange(const Player *other, QList<int> card_ids, bool chengwu) const
{
    ...
    if (card && card_ids.contains(card->getId())){
        ...
        distance_fix += w->getRange(this) - getAttackRange(false);
    }
	card = getOffensiveHorse();
    if (card && card_ids.contains(card->getId())){
        const Horse *oh = qobject_cast<const Horse *>(card);
        distance_fix -= oh->getCorrect(this);
    }
...
bool Player::hasDefensiveHorse(const QString &horse_name, bool need_area) const
{
	...
	if(d_equips.contains(horse_name)){
		...
		if(viewAsEquip(horse_name)){
			Card*ec = Sanguosha->cloneCard(horse_name);
			ec->deleteLater();
...
bool Player::hasTreasure(const QString &treasure_name, bool need_area) const
{
	...
	if(t_equips.contains(treasure_name)){
		...
		if(viewAsEquip(treasure_name)){
			Card*ec = Sanguosha->cloneCard(treasure_name);
			ec->deleteLater();
			return !isEquipsNullified(ec);
		}
	}
...
{
    return isKongcheng() && equips.isEmpty();
}
...
bool Player::canDiscard(const Player *to, const QString &flags) const
{
    ...
    if (flags.contains("h")){
		foreach(const Card *h, to->getHandcards())
			if (canDiscard(to,h->getId())) return true;
	}
    if (flags.contains("e")){
		foreach(const Card *e, to->getEquips())
			if (canDiscard(to,e->getId())) return true;
	}
    if (flags.contains("j")){
		foreach(const Card *j, to->getJudgingArea())
			if (canDiscard(to,j->getId())) return true;
	}
    return false;
}
...
bool Player::canDiscard(const QString &flags, const Player *to) const
{
	...
	return canDiscard(to,flags);
}

bool Player::canDiscard(int card_id, const Player *to) const
{
	if(to==nullptr) to = this;
...
int Player::getCardCount(bool include_equip, bool include_judging) const
{
    ...
    if (include_equip) count += equips.length();
...
void Player::setPileCardPresent(const QString &pileName, int cardId, bool present)
{
    if (pileName.isEmpty() || cardId < 0) return;
    // Reconcile one named destination reservation without touching other piles
    // or emitting a second physical move; CardMovementService owns that commit.
    auto it = piles.find(pileName);
    if (present) {
        if (it != piles.end() && it->contains(cardId)) return;
        piles[pileName].append(cardId);
    } else {
        if (it == piles.end() || it->removeAll(cardId) == 0) return;
    }
    emit gameplay_property_changed();
}
...
{/*
	...
	if(classnames.isEmpty()){
		foreach(const Slash *slash, Sanguosha->findChildren<const Slash *>()){
			if (classnames.contains(slash->getClassName())) continue;
			classnames << slash->getClassName();
		}
	}
    int count = 0;
...
QString Player::getSkillDescription(const Player *viewer) const
{
	...
	while(relateds.length()>0){
		QString sn = relateds.first();
		relateds.removeAll(sn);
		if(hasSkill(sn, true)) continue;
		const Skill *sk = Sanguosha->getSkill(sn);
		if (sk&&sk->isVisible()) {
			QString oracle = sk->getOracleText(this);
			if (!oracle.isEmpty()) {
				description += QString("<font color=\"#01A5AF\"><b>%1</b>：%2</font><br/><font color=\"#bab8ba\">%3</font><br/><br/>").arg(Sanguosha->translate(sn)).arg(sk->getDescription(this)).arg(oracle);
			} else {
				description += QString("<font color=\"#01A5AF\"><b>%1</b>：%2</font><br/><br/>").arg(Sanguosha->translate(sn)).arg(sk->getDescription(this));
			}
		}
	}
    QStringList effects;
...
bool Player::canSlashWithoutCrossbow(const Card *slash) const
{
	if(!slash){
		Slash *newslash = new Slash(Card::NoSuit, 0);
		newslash->addSubcards(getEquips(0));
		newslash->deleteLater();
		slash = newslash;
	}
...
bool Player::isCardLimited(const Card *card, Card::HandlingMethod method, bool isHandcard) const
{
    ...
    if (card->inherits("SkillCard") && method == card->getHandlingMethod()){
        ...
        if(method==Card::MethodUse&&card->inherits("Peach")&&getMark("Global_PreventPeach")>0)
			return true;
		foreach(QString pattern, card_limitation[method]){
			pattern.chop(2);
            if (isHandcard) pattern.replace("hand", ".");
...
void Player::copyFrom(Player *p)
{
    ...
    weapon = p->weapon;
    armor = p->armor;
    defensive_horse = p->defensive_horse;
    offensive_horse = p->offensive_horse;
    treasure = p->treasure;*/
    equip_area = p->equip_area;
    equips = p->equips;
    face_up = p->face_up;
    chained = p->chained;
    judging_area = QList<const Card*>(p->judging_area);
...
bool Player::isNostalGeneral(const Player *p, const QString &general_name)
{
    ...
    if (nostalMark.isEmpty())
        nostalMark << "nos_" << "tw_";
    foreach(QString s, nostalMark){
        QString nostalName = s + general_name;
        if (p->getGeneralName().contains(nostalName) || p->getGeneral2Name().contains(nostalName))
            return true;
    }
...
{
    if (role=="renegade"||fri->getRole()=="renegade") return this == fri;
    if (role.isEmpty()||fri->getRole().isEmpty()) return false;
...
{
	foreach(const Player*p, getAliveSiblings(true)){
		if(p->hasFlag("CurrentPlayer")) return true;
	}
    return false;
}
...
{
    if(hands.length()!=handcards.length()) return;
	handcards.clear();
...
{
    return isAlive() && isWounded() && !isRemoved() && getMark("command5_effect") == 0
        && !tag.value("CannotRecover").toBool();
}

bool Player::canTransform() const
{
    return Config.EnableHegemony && isAlive() && getActualGeneral2();
}

bool Player::cheakSkillLocation(const QString &skillName, bool head) const
{
    return head ? inHeadSkills(skillName) : inDeputySkills(skillName);
}

bool Player::cheakSkillLocation(const QString &skillName, const QVariant &shown) const
{
    if (!hasShownSkill(skillName)) return false;
    const QStringList skillSlots = shown.toStringList();
    return (skillSlots.contains("head") && hasShownGeneral() && inHeadSkills(skillName))
        || (skillSlots.contains("deputy") && hasShownGeneral2() && inDeputySkills(skillName));
}

QString Player::getHegemonyKingdom() const
{
    const QString chosen = tag.value("HegemonyKingdom").toString();
    if (!chosen.isEmpty()) return chosen;
    if (!actual_general1) return QString();
    if (actual_general2) {
        const QStringList kingdoms = actual_general1->compareKingdomsWith(actual_general2);
        if (!kingdoms.isEmpty()) return kingdoms.first();
    }
    return actual_general1->getKingdom();
}

void Player::setHegemonyKingdom(const QString &kingdom)
{
    tag["HegemonyKingdom"] = kingdom;
}
...
{
    const QString target = kingdom.isEmpty() ? getSeemingKingdom() : kingdom;
    QList<const Player *> players = getAliveSiblings();
    if (isAlive()) players.prepend(this);
    int count = 0;
    for (const Player *player : players) {
        const QString faction = player->getSeemingKingdom();
        if (faction.isEmpty()) continue;
        if (faction == QLatin1String("careerist")) {
            if (target == QLatin1String("careerist")) count = 1;
        } else if (faction == target) {
            ++count;
        }
    }
    // Preserve legacy identity-room Hongfa prediction. New Hegemony counts
    // actual seats; PeaceSpell accounts for heavenly_army in its own hand limit.
    if (!Config.EnableHegemony && reason != QLatin1String("AI") && type == MaxCardsType::Max
...
QStringList Player::getBigKingdoms(const QString &reason, MaxCardsType::MaxCardsCount type) const
{
    ...
    QStringList biggest;
    int maximum = 1;
    QStringList factions = Sanguosha->getKingdoms();
    for (const Player *player : players) {
        const QString faction = player->getSeemingKingdom();
        if (faction.startsWith("careerist_") && !factions.contains(faction)) factions << faction;
    }
    for (const QString &kingdom : factions) {
    ...
    if (jadeSealOwner) {
        ...
        biggest << (jadeSealOwner->getRole() == QLatin1String("careerist")
            ? jadeSealOwner->objectName() : jadeSealOwner->getSeemingKingdom());
    }
    return biggest;
}
...
{
    if (isChained() || !hasArmorEffect(QStringLiteral("IronArmor"), source)) return true;
    ...
    return biggest.contains(getRole() == QLatin1String("careerist") ? objectName() : getSeemingKingdom());
}
...
QSet<QString> Player::getPreshowedSkillInstances() const
{
    QMutexLocker locker(&m_preshowMutex);
    return m_preshowedSkillInstances;
}

bool Player::replacePreshowedSkillInstances(const QSet<QString> &instances)
{
    QSet<QString> normalized;
    for (const QString &key : instances) {
        QString baseName;
        const int instanceId = SkillInstanceUtils::parseName(key, baseName);
        if (instanceId <= 0
            || SkillInstanceUtils::formatName(baseName, instanceId) != key)
            continue;
        const SkillInstance *instance = findSkillInstance(baseName, instanceId);
        if (instance == nullptr || instance->source != SourceInnate)
            continue;
        normalized.insert(key);
    }

    QMutexLocker locker(&m_preshowMutex);
    if (m_preshowedSkillInstances == normalized)
        return false;
    m_preshowedSkillInstances = normalized;
    locker.unlock();
    // A snapshot is one state mutation even when several instance keys change.
    emit skill_state_changed();
    return true;
}
...
{
    return hasShownSkill(skill->objectName());
}
