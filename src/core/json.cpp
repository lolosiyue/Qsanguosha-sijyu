...
bool JsonUtils::isStringArray(const QVariant &var, unsigned from, unsigned to)
{
	...
	if((unsigned)array.length() > to){
		for (unsigned int i = from; i <= to; i++) {
			if (array.at(i).canConvert<QString>()) continue;
			return false;
		}
		return true;
	}
...
bool JsonUtils::isNumberArray(const QVariant &var, unsigned from, unsigned to)
{
	...
	if((unsigned)array.length() > to){
		for (unsigned int i = from; i <= to; i++) {
			if (array.at(i).canConvert<int>()) continue;
			return false;
		}
		return true;
	}
...
JsonDocument JsonDocument::fromJson(const QByteArray &json, bool allowComment)
{
    ...
    JsonDocument doc;
	doc.valid = error.error == QJsonParseError::NoError;
    if (doc.valid) doc.value = jsondoc.toVariant();
    else doc.error = error.errorString();
    return doc;
}
