...
public:
    ...
    enum HandlingMethod
    {
		...
		MethodPindian, MethodIgnore, MethodEffect, MethodPlay, MethodMove
    };
    ...
    void addMark(const QString &mark, int add_num = 1) const;
    void removeMark(const QString &mark, int remove_num = 1) const;
    virtual void setMark(const QString &mark, int value) const;
    int getMark(const QString &mark) const;
    QStringList getMarkNames() const;

    void addChange(const Card *card);
...
public:
    DummyCard(const QList<int> &subcards = QList<int>());
