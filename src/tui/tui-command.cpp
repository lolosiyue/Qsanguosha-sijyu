                             ...
                             QString *error)
{
    if (error != nullptr)
        error->clear();
    if (intent == nullptr)
        return reject(error, tuiText("tui_error_null_intent"));
    *intent = TuiCommandIntent();

    const QString input = line.trimmed();
    if (!input.startsWith(QLatin1Char('/')))
        return reject(error, tuiText("tui_error_command_prefix"));
    if (input.size() > 4096)
        return reject(error, tuiText("tui_error_command_too_long"));
    ...
    if (found == commandTable().cend())
        return reject(error, tuiText("tui_error_command_unknown").arg(keyword));

    TuiCommandIntent parsed;
    parsed.type = found.value();
    if (rejectsArgument(parsed.type) && !argument.isEmpty())
        return reject(error, tuiText("tui_error_command_no_args").arg(keyword));
    if (parsed.type == TuiCommandType::Chat) {
        if (argument.isEmpty())
            return reject(error, tuiText("tui_error_chat_empty"));
        if (argument.size() > 1000)
            return reject(error, tuiText("tui_error_chat_too_long"));
        ...
        if (argument.isEmpty()
            ...
            if (!ok || parsed.count <= 0 || parsed.count > 64)
                return reject(error, tuiText("tui_error_addrobot_args"));
        }
    }
    *intent = parsed;
    return true;
