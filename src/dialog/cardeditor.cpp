...
public:
	SkillTitle(const QString &kingdom, const QString &text)
		:title_text(nullptr), frame(nullptr)
	{
...
{
	QString suit_name = Card::Suit2String((Card::Suit)index);
	QString suit_path = QString("image/system/cardsuit/%1.png").arg(suit_name);
...
CardScene::CardScene()
	:QGraphicsScene(QRectF(0, 0, 366, 514)), menu(nullptr)
{
	photo = nullptr;
...
void CardScene::setFrame(const QString &kingdom, bool is_lord)
{
	...
	foreach (QGraphicsPixmapItem *item, magatamas) {
		item->setPixmap(QPixmap(QString("diy/%1-magatama.png").arg(is_lord ? "god" : kingdom)));
	}
...
void CardScene::keyPressEvent(QKeyEvent *event)
{
	...
	if (event->key() == Qt::Key_D) {
		//QMessageBox::information(nullptr, "", QString("%1, %2").arg(skill_box->x()).arg(skill_box->y()));
	}
}
...
{
	QGraphicsRectItem *avatar_rect = nullptr;
...
void CardEditor::updateButtonText(const QFont &font)
{
	...
	if (dialog) {
		QPushButton *button = dialog2button.value(dialog, nullptr);
		if (button)
...
void CardEditor::saveAvatar(const QRectF &rect)
{
	...
	if (!filename.isEmpty()) {
		...
		static QBitmap mask("diy/mask.png");
		pixmap.setMask(mask);
...
QWidget *CardEditor::createSkillBox()
{
	...
	for (int i = 0; i < 4; i++) {
		...
		QIcon suit_icon(QString("image/system/cardsuit/%1.png").arg(suit_name));
...
{
	static CardEditor *editor = new CardEditor(this);
