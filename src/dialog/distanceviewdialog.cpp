...
public:
    DistanceViewDialogUI()
    {
        ...
        foreach (const DistanceSkill *skill, Sanguosha->getDistanceSkills()) {
            foreach (const ClientPlayer *p, ClientInstance->getPlayers()) {
                if (p->hasSkill(skill->objectName(), true)) {
    ...
    : QDialog(parent)
{
    ...
    foreach(QLineEdit *edit, ui->distance_edits) {
        ...
        if (Sanguosha->translate(skill_name).startsWith("#")) {
            const Skill *main_skill = Sanguosha->getMainSkill(skill_name);
            if (main_skill) skill_name = main_skill->objectName();
        }
