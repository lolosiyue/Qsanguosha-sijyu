...
void ServerInfoWidget::fill(const ServerInfoStruct &info, const QString &address)
{
	...
	foreach (const Package *package, Sanguosha->getPackages()) {
        ...
        bool disabled = info.BanPackages.contains(package->objectName());
		QString package_name = Sanguosha->translate(package->objectName());
        QCheckBox *checkbox = new QCheckBox(package_name);
        checkbox->setChecked(!disabled);

        new QListWidgetItem(disabled ? disabled_icon : enabled_icon, package_name, list_widget);
    }
}
...
{
    if (lack_label)
        lack_label->setPixmap(QPixmap(QString("image/system/number/%1.png").arg(count)));
}
