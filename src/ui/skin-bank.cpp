...
#else
	// Android: Use Qt's built-in text rendering with vertical text support
	painter->setPen(m_color);
	QFont font;
	font.setPixelSize(m_fontSize.height());
	painter->setFont(font);

	if (m_vertical && text.length() > 1) {
		...
		if (align & Qt::AlignVCenter) {
			startY = pos.y() + (pos.height() - totalHeight) / 2;
		} else if (align & Qt::AlignBottom) {
			startY = pos.y() + pos.height() - totalHeight;
		}

		int startX = pos.x();
		if (align & Qt::AlignHCenter) {
			startX = pos.x() + (pos.width() - fm.horizontalAdvance(text.at(0))) / 2;
		} else if (align & Qt::AlignRight) {
			startX = pos.x() + pos.width() - fm.horizontalAdvance(text.at(0));
		}
		...
		for (int i = 0; i < text.length(); i++) {
			...
			painter->drawText(startX, charY + fm.ascent(), QString(ch));
		}
	} else {
		// Horizontal text: use normal rendering
	...
	QSanButton::ButtonState state) const
{
	...
	if (path.isEmpty()) return QPixmap(1, 1); // older Qt version cries for non-zero QPixmap...
...
QPixmap QSanRoomSkin::getProgressBarPixmap(int percentile) const
{
	...
	if (allMaps_var.canConvert<JsonArray>()){
		...
		for (int i = 0; i < allMaps.size(); i++) {
			...
			&&allMaps[i].value<JsonArray>()[0].toInt() >= percentile) {
...
QPixmap QSanRoomSkin::getGeneralPixmap(const QString &generalName, GeneralIconSize size) const
{
	...
	if (size == S_GENERAL_ICON_SIZE_CARD)
		...
		return getPixmap(QString(S_SKIN_KEY_PLAYER_GENERAL_ICON).arg(size), name);
	}
}
...
QString QSanRoomSkin::getPlayerAudioEffectPath(const QString &eventName, const QString &category, int index) const
{
	...
	if (index < 1)
		fileName = getRandomAudioFileName(fileName);
	else {
		QStringList fileNames = getAudioFileNames(fileName);
                                                          ...
                                                          const QString &generalName, int skinIndex) const
{
	...
	if (skinIndex > 0) {
		...
		QString heroskinSkillPath = QString("hero-skin/%1/%2").arg(actualGn).arg(skinIndex);
		QStringList filters;
		filters << "*.ogg" << "*.wav";
...
bool IQSanComponentSkin::AnchoredRect::tryParse(const QVariant &var)
{
	...
	if (JsonUtils::isNumberArray(value, 0, 3)) {
		QRect rect;
		if (!JsonUtils::tryParse(value, rect)) return false;
		m_useFixedSize = true;
	...
	const QString &audioConfigName, const QString &animationConfigName)
{
	...
	if (!imageConfigName.isEmpty()) {
		...
		if (imageDoc.isValid()&&imageDoc.isObject()) {
			...
			QString errorMsg = QString("Error when reading image config file \"%1\": \n%2") .arg(imageConfigName).arg(imageDoc.errorString());
			QMessageBox::warning(nullptr, "Config Error", errorMsg);
			success = false;
		}
	}

	if (!audioConfigName.isEmpty()) {
		...
		if (audioDoc.isValid()&&audioDoc.isObject()) {
			...
			QString errorMsg = QString("Error when reading audio config file \"%1\": \n%2").arg(audioConfigName).arg(audioDoc.errorString());
			QMessageBox::warning(nullptr, "Config Error", errorMsg);
			success = false;
		}
	}

	if (!animationConfigName.isEmpty()) {
		...
		if (animDoc.isValid()&&animDoc.isObject()) {
			...
			QString errorMsg = QString("Error when reading animation config file \"%1\": \n%2").arg(animationConfigName).arg(animDoc.errorString());
			QMessageBox::warning(nullptr, "Config Error", errorMsg);
			success = false;
		}
	}
...
QPixmap IQSanComponentSkin::getPixmap(const QString &key, const QString &arg, bool cache) const
{
	...
	QString totalKey, fileName = key;
	bool clipping = false, scaled = false;
	QRect clipRegion;
	QSize scaleRegion;

	// case 1 and 2
	if (arg.isEmpty()) totalKey = key;
	...
	else if(!arg.isEmpty()){ // case 3: use default
...
bool QSanSkinFactory::switchSkin(QString skinName)
{
	...
	if (skinName != S_DEFAULT_SKIN_NAME) success = _sm_currentSkin.load(_m_skinList[skinName.toLatin1().constData()]);
	if (!success) qWarning("Loading skin %s failed", skinName.toLatin1().constData());
	return success;
}
