...
{
    if (!_ftLibInitialized || font == nullptr || painter == nullptr || text.isEmpty())
        return false;
...
#define _FONT_PIXEL(x, y) (bitmap.buffer[(y) * bitmap.pitch + (x)])
    ...
    for (int i = 0; i < len; i++) {
        ...
        int fontRows = bitmap.rows, fontCols = bitmap.width;
        ...
        for (int y = 0; y < fontRows; y++) {
            ...
            int fontClippedCols = cols - 1 - currentX;
