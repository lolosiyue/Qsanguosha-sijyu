...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (damage.from&&damage.from->getEquips().length() <= qMin(2, player->getEquips().length())) {
			room->broadcastSkillInvoke(objectName());
...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *kongrong, QVariant &data) const
    {
        ...
        if (move.to_place == Player::DiscardPile && move.from == kongrong
            && ((move.reason.m_reason & CardMoveReason::S_MASK_BASIC_REASON) == CardMoveReason::S_REASON_DISCARD)) {

            int i = 0;
            QList<int> lirang_card;
            foreach (int card_id, move.card_ids) {
                if (!room->getCardOwner(card_id)
                    && (move.from_places[i] == Player::PlaceHand || move.from_places[i] == Player::PlaceEquip)) {
                    lirang_card << card_id;
                }
                i++;
            }
            if (lirang_card.length()>0&&kongrong->askForSkillInvoke(objectName()+"$-1",data))
				kongrong->assignmentCards(lirang_card,"lirang|@lirang-distribute",room->getOtherPlayers(kongrong),-1,1);
        }
        return false;
    }
};
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == Dying) {
            ...
            if (dying.who != player && target
                && target->askForSkillInvoke(this,QString("draw:%1").arg(player->objectName()),target==player)) {
                room->broadcastSkillInvoke(objectName(), 1);
                if (target != player) {
                    LogMessage log;
            ...
            if (target && target->askForSkillInvoke(this, QString("losehp:%1").arg(player->objectName()),target==player)) {
                room->broadcastSkillInvoke(objectName(), 2);
                if (target != player) {
                    LogMessage log;
                    log.type = "#InvokeOthersSkill";
...
public:
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &) const override
    {
    ...
    : Package("hegemony")
{
    ...
    skills << new CompanionVS << new HalfMaxHp << new FirstShow << new Careerman;
...
void FirstShowCard::use(Room *room, ServerPlayer *player, QList<ServerPlayer *> &targets) const
{
    ...
    foreach (const QString &name, list) {
        LogMessage log2;
        log2.type = "$KnownBothViewGeneral";
        log2.from = player;
        log2.to << to;
        log2.arg = name;
        log2.arg2 = choice;
        room->doNotify(player, QSanProtocol::S_COMMAND_LOG_SKILL, log2.toVariant());
    }
    JsonArray arg;
    arg << "firstshow";
...
void CareermanCard::use(Room *room, ServerPlayer *player, QList<ServerPlayer *> &) const
{
    ...
    } else if (choice == "draw2cards") {
        player->drawCards(2, "careerman");
    } else if (choice == "peach") {
        room->useCard(CardUseStruct(peach, player, player));
