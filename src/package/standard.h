...
public:
    ...
    //void doPreAction(Room *room, const CardUseStruct &) const;
...
private:
    bool doCollateral(Room *room, ServerPlayer *killer, ServerPlayer *victim, const QString &prompt, ServerPlayer *user) const;
};
...
public:
    ...
    int getRange(const Player *player = nullptr) const;
    void setRange(int n);
...
public:
    ...
    {
        if(different_slash) return BasicCard::objectName();
		return "slash";
    }
