...
public:
    ...
    {
        return target != nullptr && target->hasFlag("neoluoyi") && target->isAlive();
    }
...
public:
    ...
    bool onPhaseChange(ServerPlayer *gongsun, Room *room) const
    {
        if (gongsun->getPhase() == Player::Finish && gongsun->askForSkillInvoke(this)) {
            ...
            if (!room->askForCard(gongsun, ".Weapon", "@zhulou-discard"))
                room->loseHp(HpLostStruct(gongsun, 1, objectName(), gongsun));
        }
        return false;
    }
};
