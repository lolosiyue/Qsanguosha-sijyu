...
public:
    ...
    {
        return target != nullptr && target->isLord();
    }
        ...
        if (target->getPhase() == Player::Start) {
            ...
            foreach (ServerPlayer *player, others) {
                if (player->getHandcardNum() == 0) {
                    if (has_frantic)
                        room->loseHp(HpLostStruct(player, 2, "jishi", target));
                    else
                        room->loseHp(HpLostStruct(player, 1, "jishi", target));
                } else {
    ...
    bool trigger(TriggerEvent triggerEvent, Room* room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (has_frantic) {
            if (TriggerSkill::triggerable(player) && player->isWounded() && triggerEvent == TargetConfirmed) {
                ...
                if (!use.card->isKindOf("TrickCard")||player->hasFlag("CurrentPlayer")) return false;
...
public:
    ...
    void startDeadJudge(ServerPlayer *lord) const
    {
        ...
        foreach (ServerPlayer *player, players) {
            ...
            room->judge(judge);
            if (judge.isBad())
                room->loseHp(HpLostStruct(player, player->getHp(), "scenario"));
        }
    }
            ...
            if (player->isLord()) {
                if (player->getHp() <= 3 && player->getMark("@frantic") <= 0) {
                    ...
                    foreach(const Card *card, judges)
                        room->throwCard(card->getEffectiveId(), nullptr);
                }
            }
            break;
        }
