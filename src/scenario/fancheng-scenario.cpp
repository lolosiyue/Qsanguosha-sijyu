...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (damage.to->isLord()) {
            int x = damage.damage;
            room->recover(damage.to, RecoverStruct(damage.from, nullptr, x * 2, "guagu"));
...
void FloodCard::onEffect(CardEffectStruct &effect) const
{
...
{
    room->loseHp(HpLostStruct(source, 1, "taichenfight", source));
...
{
    CardMoveReason reason(CardMoveReason::S_REASON_GIVE, source->objectName(), targets.first()->objectName(), "zhiyuan", "");
