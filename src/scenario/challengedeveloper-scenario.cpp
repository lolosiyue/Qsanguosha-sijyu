...
public:
    ...
    bool onPhaseChange(ServerPlayer *player, Room *room) const
    {
        foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
            if (player->isDead()) return false;
            if (p->isDead() || !p->hasSkill(objectName()) || p->isKongcheng()) continue;
...
public:
    ...
    bool trigger(TriggerEvent event, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (event == EventPhaseStart) {
			...
			|| player == room->getCurrent() || !player->hasSkill(objectName())) return false;
...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (player == room->getCurrent()||!player->hasSkill(objectName())) return false;
        DamageStruct damage = data.value<DamageStruct>();
...
void DevGengxinCard::onEffect(CardEffectStruct &effect) const
{
    ...
    if (subcards.isEmpty()) {
        ...
        int range = weapon->getRange(effect.to);
        ...
        int range = weapon->getRange(effect.from);
        effect.from->gainHujia(range);
    }
}
...
public:
    ...
    bool onPhaseChange(ServerPlayer *player, Room *room) const
    {
        foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
            if (player->isDead()) return false;
            if (p->isDead() || !p->hasSkill(objectName()) || !player->canDiscard(p, "he")) continue;
...
public:
    ...
    bool trigger(TriggerEvent event, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (event == PreCardUsed) {
            ...
            if (data.toString() == objectName() && player->hasSkill(objectName(), true)) {
            ...
            if (data.toString() != objectName() || player->hasSkill(objectName(), true)) return false;
            foreach (QString mark, player->getMarkNames()) {
                if (mark.startsWith("&dev_juanlao+:+"))
					room->setPlayerMark(player, mark, 0);
            }
...
public:
    ...
    bool trigger(TriggerEvent event, Room *room, ServerPlayer *player, QVariant &data) const
    {
		if(event == CardUsed){
			...
			if(use.who&&use.who!=player&&player->hasSkill(objectName())&&player->askForSkillInvoke(this))
				player->drawCards(1,objectName());
			foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
				if(use.who==p&&p->isAlive()&&p->hasSkill(objectName())&&p->askForSkillInvoke(this))
					p->drawCards(1,objectName());
			}
		}else if (event == CardResponded) {
			CardResponseStruct res = data.value<CardResponseStruct>();
			if(res.m_card->getTypeId()<1||!res.m_toCard) return false;
			if(res.m_who&&res.m_who!=player&&player->hasSkill(objectName())&&player->askForSkillInvoke(this))
				player->drawCards(1,objectName());
			foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
				if(res.m_who==p&&p->isAlive()&&p->hasSkill(objectName())&&p->askForSkillInvoke(this))
					p->drawCards(1,objectName());
			}
        }
		return false;
...
public:
    ...
    {
        foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
            if (p->isDead()||!p->hasSkill(objectName())||!player->askForSkillInvoke(this,p)) continue;
			p->drawCards(1,objectName());
        }
		return false;
    }
