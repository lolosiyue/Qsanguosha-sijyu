...
bool MiniSceneRule::trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &) const
{
    if (triggerEvent == EventPhaseStart) {
    ...
    } else if (triggerEvent == GameReady) {
        ...
        foreach (int id, m_fixedDrawCards) {
            if (drawPile.contains(id)) {
                ...
                room->moveCardTo(Sanguosha->getCard(id), nullptr, Player::DrawPile, true);
            room->addPlayerHistory(nullptr, "pushPile");
        }
        ...
        for (int j = 0; j < players.length(); j++) {
            ...
            if (this->players.at(i)["chained"] != "") {
                sp->setChained(true);
                room->broadcastProperty(sp, "chained");
                room->setEmotion(sp, "chain");
            }

            if (this->players.at(i)["turned"] != "") {
                if (sp->faceUp())
                    sp->turnOver();
            }

            if (this->players.at(i)["starter"] != "") {
                room->setCurrent(sp);
                room->setTag("Starter", QVariant::fromValue(sp));
            }

            str = this->players[i]["nationality"];
			if (str != "") {
                room->setPlayerProperty(sp, "kingdom", str);
            }

            str = this->players[i]["marks"];
            if (str != "") {
            ...
            foreach (QString equip, str.split(",")) {
                bool ok;
                int id = equip.toInt(&ok);
                if (ok)
                    room->moveCardTo(Sanguosha->getCard(id), sp, Player::PlaceEquip,
            ...
            if (str != "") {
                foreach(QString judge, str.split(","))
                    room->moveCardTo(Sanguosha->getCard(judge.toInt()), sp, Player::PlaceDelayedTrick,
					CardMoveReason(CardMoveReason::S_REASON_UNKNOWN, ""));
            }

            str = this->players.at(i)["hand"];
            if (str != "") {
                DummyCard *dummy = new DummyCard(ListS2I(str.split(",")));
            ...
            if (str == "") str = "4";
            //room->drawCards(sp, str.toInt(), "InitialHandCards");
            room->setTag("FirstRound", false);
			draws << str.toInt();
        }
		room->setTag("FirstRound", true);
		room->drawCards(players, draws, "InitialHandCards");
		room->setTag("FirstRound", false);
        foreach (ServerPlayer *sp, players) {
			room->getThread()->trigger(GameStart, room, sp);
		}
