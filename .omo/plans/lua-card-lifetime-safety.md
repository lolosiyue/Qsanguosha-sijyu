...
| `QObject::deleteLater()` is not virtual | 承認非 virtual 且 worker 無 event loop，因此不能保證 base-typed call 即時進入 Card policy；支援入口鎖定為 `Card*`／衍生 Card slot 及 Card meta-object call；`QObject*`／qualified base call 在 production source checker 中視為 bypass，PR7 前必須為零；`Card::event(DeferredDelete)`只作「事件確實被 dispatcher 投遞時」的防線；ordinary non-Card QObject timing stays unchanged |
...
| D. Lua-owned transient | Card constructor wrapper with SWIG `own=1`, 例如 `sgs.DummyCard()` | `LuaOwnedTransient` with owning-wrapper lease | manager-aware Card `__gc` or explicit delete request; not orphan-reaped while a live owning wrapper remains, and never stock-direct deleted after adoption/death |
...
**N5.6 Terminal shutdown — unique idempotent `RoomRuntime::shutdownFinal()` (C6):** 全進程唯一終局關閉入口，冪等（idempotent，首個呼叫原子置位 `Closing`，重入為 no-op），順序固定、與解構次序無關。僅由 `Room::~Room()`（及替代的 Room 擁有者析構路徑）呼叫；`RoomRuntime::~RoomRuntime()` 僅委託至此函式，不另做清理。
...
| 6 | unregister／postclose | 從 process registry 移除 runtime 關聯；在 owner thread 執行 postclose drain：處理最後零 wrapper orphan，校驗 per-runtime pending/unclaimed/wrapper/lease counters 回到 baseline，並輸出每 domain evidence snapshot（**不得輸出 `FINAL_GAUGE` marker，marker 僅於第 8 步 final-zero assertion 後唯一輸出**） |
| 7 | 顯式冪等清除 RoomState／definition Card owners | 在 `shutdownFinal()` 內顯式、冪等清除 `RoomState`／`WrappedCard`／definition 擁有者所屬 Cards（於 canonical owner `Room::thread()` 親和上），不得等待函式返回後的正常成員析構；`Room::~Room()`／`RoomRuntime::~RoomRuntime()` 僅委派同一冪等 cleanup 或斷言已清除；因 process registry 仍存活，析構期 `invalidateIfObserved` 安全，即使 Lua 已關閉 |
| 8 | final-zero assertion 與唯一 FINAL_GAUGE | 終局斷言：process registry 仍存活期間，校驗所有 managed/token/wrapper/unclaimed/lease/explicit-owner/sidecar-edge 及 per-runtime invocation/closing counters 為 baseline；`actually_destroyed` 等單調計數保持一致；無 manager 鎖內 Lua/Qt/Card 呼叫；**完成 final-zero assertion 後，才於 canonical owner 上輸出唯一 `FINAL_GAUGE` marker（`[CardLifetime] FINAL_GAUGE {...}` JSON），Marker 不得在 postclose／第 6 步提前輸出** |

* 任何非 `shutdownFinal()` 路徑不得重排或省略上述順序；解構次序推斷不可替代顯式呼叫。
* `RoomRuntime::shutdownFinal()` 冪等：以原子 flag 保護，重入直接返回；第 7 步在 `shutdownFinal()` 內顯式冪等清除（不得等待成員析構），析構函式僅委派同一冪等 cleanup 或斷言已清除；第 8 步完成 final-zero assertion 後才輸出唯一 `FINAL_GAUGE` marker，Marker 不得在 postclose／第 6 步提前輸出；`RoomRuntime::~RoomRuntime()` 僅 `if (!closed) shutdownFinal()`。

**N5.7 AI init-failure／restart shutdown 與 terminal shutdown 分離 (C6):** 以下兩者與 N5.6 終局路徑嚴格分離，不得共用同一函式或重排步驟：

* `AI init-failure`：`RoomRuntime::initialize()` 內 `m_ai.initialize()` 失敗時，僅走 `AiLuaRuntime::shutdownForInitFailure()`（或替代的局部回滾）：關閉已部分建立的 AI Lua 狀態、釋放 AI-side 臨時資源、保留 game Lua 及 Room 生命週期；**不**執行 N5.6 的步驟 1/3/4/6/7/8，不清除尚未建立的 persistent roots，不對 game domain 做 pre/postclose drain。若 game Lua 本身初始化失敗，則走 `RoomRuntime::shutdownForInitFailure()` 的 game 失敗分支：僅清理已建立的 game Lua 及已配置的 registry 條目，不觸及 worker join（workers 尚未啟動）。
...
| native Card deletion ingress | Card-specific public-slot `deleteLater()` 僅支援 `Card*`／衍生 Card slot 及 Card meta-object call 進入 native-origin policy（`QObject::deleteLater()` is not virtual 且 worker 無 event loop，不能保證 base-typed call 即時進入 Card policy）；`QObject*`／qualified `QObject::deleteLater` 在 production source checker 中視為 bypass，PR7 前必須為零；`Card::event(DeferredDelete)`只作「事件確實被 dispatcher 投遞時」的防線；ordinary non-Card QObject `deleteLater()` is unchanged；base destructor always invalidates aliases for unavoidable direct `delete` owners | preserves current C++ spelling; 僅 Card*／衍生／meta-object 為受支援入口；QObject*／qualified base-call 為 bypass |
...
|---:|---|---|---|
| 1 | Add a Card-specific public-slot C++ `Card::deleteLater()` with the existing spelling; 承認 `QObject::deleteLater()` is not virtual 且 worker 無 event loop，不能保證 base-typed call 即時進入 Card policy；支援入口鎖定為 `Card*`／衍生 Card slot 及 Card meta-object call；`QObject*`／qualified base call 視為 bypass，PR7 前必須為零；`Card::event(DeferredDelete)`只作「事件確實被 dispatcher 投遞時」的防線；leave ordinary non-Card QObject deletion unchanged. | approved 2026-08-22 | A Lua-only bridge cannot cover the large native Card deletion surface. Non-virtual plus no event loop admits base-typed bypass. |
...
| Native compatibility ingress | representative `Card*`, `SkillCard*`, and derived-pointer C++ `Card::deleteLater()` calls 以及 Card meta-object call 進入 native-origin policy；`QObject* q = card; q->deleteLater()`／qualified `QObject::deleteLater` 為 production bypass（source checker 計 `card_delete_bypass`，PR7 前必須為零）；ordinary non-Card QObject `deleteLater()` is unchanged；`Card::event(DeferredDelete)`只作事件確實被 dispatcher 投遞時的防線 |
| Native bypass guard | qualified／member-pointer `QObject::deleteLater` on Card* 及 `QObject* q = card; q->deleteLater()` 皆由 source checker 視為 bypass（`card_delete_bypass`，PR7 前必須為零）；僅 `Card*`／衍生 slot 及 Card meta-object call 為受支援入口；`Card::event(DeferredDelete)`只作 dispatcher 確實投遞時的防線，不宣稱 worker base-call 必定被攔截；injected external owner destruction still invalidates aliases and fails if leases remain |
...
The lifetime gate has exactly four locked CTest names: `qsanguosha_card_lifetime`, `qsanguosha_card_lifetime:lua`, `qsanguosha_card_lifetime:synthetic-30`, and `qsanguosha_card_lifetime:synthetic-50`. The core/Lua suites are implemented in `tests/card-lifetime-manager-test.cpp`; PR8 adds the two synthetic suites. `card-lifetime-legacy-red` is a registered `runtime-tests-main.cpp` dispatcher child suite invoked only by the core CTest, not a separately registered CTest. Except the unique selector-default regression, each test constructs its `CardLifetimeMode` explicitly.
...
## Finding → 修正對照表 (C1–C6, H1–H6, M1–M3) — 上一輪正式審查原始 finding 追溯

> 本表 C1–C6、H1–H6、M1–M3 嚴格對應上一輪正式審查原始 finding ID，不得重新定義 ID；每項列出規範章節、PR、驗收證據。現有 Card 行為缺陷清單另以 B1、B2…… 列示，不可佔用原 finding ID。

| ID | 嚴重度 | 問題簡述（原始審查 finding） | 規範章節 / PR / 驗收證據 |
|---|---|---|---|
| C1 | Critical | 正式計畫不是自足規格，曾把 state、ownership、adoption、shutdown 與 bounded formulas 留在 draft | Scope、Normative authority、N1–N14；PR1–PR8；驗收以本檔完整表格及歷史草案規範引用零命中為證，executor 不需外部 draft |
| C2 | Critical | Lazy observation 無法同時保證未觀察 Card 零 token 與 post-destruction 首次 wrapper 安全 | N7、N10、N14；PR2；所有 exposure 必須在有效 owner/borrow/invocation scope 內 `observeLive`，destructor-won 拒絕 wrapper；明確排除 post-free untracked first-wrapper，測試不得加入 eager token/tombstone |
| C3 | Critical | Keyed sidecar 無法單獨追蹤 detached/nested QVariant 與 event struct copies | N8.3、N8.5；PR1 ledger、PR5；frozen metatype matrix、copy/move/overwrite/dtor、detached/nested/opaque focused tests及 `unknown_qvariant_card_payload==0` |
| C4 | Critical | WrappedCard adoption 原 target 可能是已退出且無 dispatcher 的 setup thread | N5 setup normalization、N9；PR1 dispatcher-liveness characterization、PR5 adoption、PR6 normalization、PR7 `RoomState::reset/resetCard` integration test；canonical owner 鎖定 `Room::thread()`、lease-before-move、post-move no-fail |
| C5 | Critical | 長生命 `LuaRuntime::Binding` 與短生命 Lua invocation 混用會令 safe point 永遠被 depth 阻塞 | N5.1–N5.3、N7-5；PR2 `LuaInvocationScope`、PR6 drains；outermost `lua_pcall` pin/longjmp、normal/TurnBroken/StageChange/extra-turn tests |
| C6 | Critical | 缺少唯一且安全的 terminal shutdown owner，close-time GC、registry 與 physical owners 次序不明 | N5.4–N5.7；PR6；唯一冪等 `RoomRuntime::shutdownFinal()` 八步序列、init-failure/restart 分離、step 7 顯式 owner cleanup、step 8 final zero 後唯一 marker |
| H1 | High | `Player::equips` 與公開 mutable `change_cards` 契約不可直接實作 | N8.1、N8.2；PR4 storage/consumer migration、PR5 blockers；`QList<const WrappedCard *>`、query-time resolver、borrow lifetime、generation/duplicate/cycle/source-dtor/address-reuse tests |
| H2 | High | Persistent owner 與 raw destruction ledger 未閉合 | N8.4、N8.5；PR1 ledger、PR4 owner migration、PR5 leases；Card tags、whole-map copy、`Player::copyFrom`、`ComboMovesCard`、QObject parent-child、`RoomState::reset/resetCard`、`m_ownedCard` 均有 acquire/release/test row |
| H3 | High | SWIG ingress 可被 static QObject alias、Lua-writable uservalue 或 longjmp-skipped RAII 繞過 | N7、N10、N12-1；PR2/PR3；Lua-invisible side table、outermost invocation pins、all three SWIG helper gates；QObject base calls明定為 checker bypass且 PR7 前 `card_delete_bypass==0` |
| H4 | High | 多個 PR 的 CMake、suite dispatcher 與 Lua 驗收入口在當前起點不存在或時序過早 | Verification strategy；PR1 root source/test dispatcher、PR2 Lua CTest、PR5/PR6 reclaim assertions；四個 locked CTest names及統一 regex，禁止 `debug\\QSanguosha.exe --lua-test` |
| H5 | High | PR8/F3 原命令不能固定 Debug executable，也不能驗證每 Room baseline | N6、Verification strategy；PR6 final marker、PR8 `--exe`/CSV/headless/synthetic；5/5 `20p`、marker missing/duplicate/nonzero failure及 30/50 topology/formulas |
| H6 | High | 回退契約未驗證保留 PR8 時單獨回退 PR7，以及其後的完整序列 | Commit strategy、Rollback rehearsal；PR7/PR8；final SHA 隔離 worktree 實跑正常 `8->7->...->1` 與 PR7-only 後 `8->6->...->1`，兩路均跑 locked gates |
| M1 | Medium | 「所有測試顯式選 mode」與 PR7 selector-default regression 自相矛盾 | N11；PR1–PR8；除唯一 PR7 selector-default regression 外全部顯式 mode，PR7-only rollback 只改 selector expectation |
| M2 | Medium | Legacy red oracle 若依賴 allocator crash/double-delete 會不確定 | Verification strategy；PR1/PR2；dispatcher child timeout 15s，parent 將 unsafe outcome 正規化為內部 RED code 70/JSON並在 PR1 返回 0，PR2 以 SAFE exact errors/counters 完成 red-to-green |
| M3 | Medium | Source checker 無法單獨證明 qDeleteAll、parent teardown、upcast、member pointer、macro與alias閉合 | N8.5、N8.6；PR1 checker/ledger、PR4 migration、PR7 gate；self-test blind spots＋人工有限 ledger＋N6 runtime counters共同判定，禁止以 scan clean 單獨宣稱閉合 |

> 上表每列對應審查 gate：C/H/M 任一未在指定章節實作或 PR 驗收即為阻擋項；PR7 default-on 零 gate 須同時滿足 `unknown_unclaimed==0` 等 N6 指標。

### Card 行為缺陷清單（B1–B15，非審查 finding，僅作實作背景）

| ID | 嚴重度 | 問題簡述 | 規範章節 / 實作 PR / 關鍵檔案 |
|---|---|---|---|
| B1 | Critical | SWIG alias 在 C++ Card 析構後仍可轉換並解引用（stale UAF） | N7-1/2/4、N10 觀察/失效線性化；PR2；`swig/sanguosha.i:47-85` 三 helper 重定義、`src/core/card.cpp:43-49` 析構 `invalidateIfObserved`、`builds/cmake-vs2026/generated/sanguosha_wrap.cxx` 檢查 |
| B2 | Critical | `sgs.DummyCard()` 等 `own=1` 擁有型 userdata 在 `__gc` 二次刪除 | N7-3/7、N3 `LuaOwnedTransient`、N10 original-own 位；PR2；`swig/sanguosha.i` `__gc` 分發、`src/core/card-lifetime-manager.{h,cpp}` owning wrapper 計數 |
| B3 | Critical | `Player::equips` 存 `getRealCard()` 內指針，`WrappedCard::takeOver` 後懸空 | N8.1 內存改 `QList<const WrappedCard *>`、查詢時 `getRealCard()`；PR4；`src/core/player.h:507`、`src/core/player.cpp:882-895,948-1088` 全遷移 |
| B4 | Critical | `WrappedCard::takeOver` 無法撤銷已排程的 pending delete，舊隊列誤刪新 `m_card` | N9 交易式收養、N2 `Adopting` 撤銷隊列；PR5；`src/core/wrapped-card.cpp:3-83` reserve/cancel |
| B5 | Critical | Lua 可經 `lua_setuservalue` 偽造 token 世代/original-own | N7-3/4/6、N10 權威欄位在 Lua 不可見 side table + C++ token；PR2；`src/core/lua-runtime.*` side table、`swig/sanguosha.i` ConvertPtr 校驗 |
| B6 | Critical | 析構與 `observeLive` 競爭：未追蹤裸指針在析構後首次建 wrapper 復活舊世代 | N10 單一 registry mutex 線性化、N7-2 `observeLive` 僅在活域內且 destructor-won 拒絕；PR2；`src/core/card-lifetime-manager.{h,cpp}` |
| B7 | High | `RoomThread::run` 無 `exec()`，worker 親和 `deleteLater` 堆積至線程退出 | N5.4 worker-final 嚴格 8 步順序、N5.6 `shutdownFinal()`；PR6；`src/server/roomthread.cpp:603-685`、`src/server/roomthread1v1.cpp:20-113`、`roomthread3v3.cpp:64-116`、`roomthreadxmode.cpp:19-115` |
| B8 | High | 原生 `Card::deleteLater()` 為主流量入口，僅攔 Lua 仍被繞過 | N12-1、N3 來源區分、N6 `native_delete_requested`；PR3；`src/core/card.h` 新增 `Card::deleteLater()` slot、`src/core/card.cpp` `Card::event` 僅 dispatcher 投遞時防線（非 virtual，不保證 base-call 攔截） |
| B9 | High | `delete Card*` / `qDeleteAll` / 預設智能指針 deleter 繞過 lease/owner 檢查 | N8.4/8.5/8.6、`docs/card-lifetime-ownership.md` 陵記；PR4 遷移；`tools/check-card-lifetime.py --self-test` 含 qDeleteAll/upcast/member-pointer/macro 盲點 |
| B10 | High | `safeTurnCardToEquip` 四失敗分支孤兒泄漏（已知工廠未配對退出） | N2 `UnclaimedTransient -> Retired` 孤兒邊、N4 `unknown_unclaimed` gate；PR3 首個孤兒 fixture、PR5 `ManagedReclaim` 回收；`src/package/yjcm2023.cpp:29-44` |
| B11 | High | `Card::Parse` 克隆後已排 `deleteLater` 仍回傳裸指針（同棧使用/收養語意不清） | N1-C/E 分類、N3 `PendingDelete` grace、N2 same-stack 可用；PR3；`src/core/card.cpp:565-680` Parse 註冊 |
| B12 | High | `CardFinished` / decision return / `event_stack.isEmpty()` 被誤作回收證明 | N5.2/N5.3 列舉安全路徑、N5.1 `LuaInvocationScope`；PR6；`src/server/room.cpp:223-294` phase/turn hook 僅 proposal |
| B13 | Medium | 原始指標地址重用導致 stale 與新 Card 相等性混淆 | N7-6 `__eq` token 世代比對、N6 `change_list_reuse_reconnect`；PR2；`swig/sanguosha.i` `__eq` 分發、確定性 allocator 重用 fixture |
| B14 | Medium | `WrappedCard` 替換/析構缺親和性與空值契約（泄漏/double delete/跨線程） | N9 步驟表、N5.4 adopt 跳過、N5 setup normalization；PR5/PR6；`src/core/room-state.cpp:5-58`、`src/core/wrapped-card.cpp` |
| B15 | Medium | `QVariant`/`m_extraData`/tag 內 Card 指針經通用容器逃逸，無提取/釋放策略 | N8.3 凍結矩陣、N8.5 ledger 必列、N6 `unknown_qvariant_card_payload==0`；PR5；`src/core/structs.h:50-177,473-606` 每型別複製/覆寫/析構鉤 |
...
- Focused gate（全 PR 統一）: `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure`（lifetime 套件鎖定為 `qsanguosha_card_lifetime`、`qsanguosha_card_lifetime:lua`、`qsanguosha_card_lifetime:synthetic-30`、`qsanguosha_card_lifetime:synthetic-50`，全 lifetime gate 統一使用 regex `^qsanguosha_card_lifetime(:.*)?$`）. Cumulative gate 亦統一為 `ctest --test-dir builds/cmake-vs2026 -C Debug --output-on-failure`. 若回傳 `0xc0000135`，先 `set PATH=H:\Qt6111\6.11.1\msvc2022_64\bin;%PATH%` 重跑一次再判定語意失敗。
- SWIG gate after any `.i` edit: incremental build 後執行 `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure`（涵蓋具名 Lua CTest `qsanguosha_card_lifetime:lua`），再檢查 `builds/cmake-vs2026/generated/sanguosha_wrap.cxx` 是否含三個受檢分發（`SWIG_NewPointerObj`／`SWIG_ConvertPtr`／`SWIG_MustGetPtr` 之重定義）且 `git diff -- swig/sanguosha_wrap.cxx` 為空。禁止使用 `debug\QSanguosha.exe --lua-test` 作為驗收。
...
- Legacy red oracle（用於 PR2 之前的紅態特徵）: `card-lifetime-legacy-red` 是已註冊於 `runtime-tests-main.cpp` dispatcher、可由 `qsanguosha_card_lifetime` 啟動的 child suite，但不另以 `add_test`／`qsan_add_ctest` 註冊成 CTest。Core parent 以 `ObserveOnly`、單進程、timeout `15s` 啟動 child，將 crash／timeout／錯誤文字或計數不符正規化為 JSON `{"suite":"card-lifetime-legacy-red","classification":"RED","normalized_exit":70,"child_exit":N,"timed_out":false,"error":"...","actually_destroyed":N}`；`70` 是 parent 內部斷言的 RED classification code，不是已提交 CTest 的程序退出碼。PR1 parent 預期 `classification=RED` 後返回 0；PR2 先把 parent 期望改為 SAFE 並保留 pre-fix 非零 red evidence，實作後要求 child／parent exit 0、精確錯誤文字 `Lua error: attempt to use deleted Card`／`Lua error: attempt to delete Card with unknown ownership` 及 `actually_destroyed==1`／`stale_access==1`。Unknown-suite exit 64 僅是獨立 dispatcher 負向測試。
## Execution strategy
...
|---:|---|---|
| 1 Card-specific C++ slot plus explicit QObject base-call exclusion | PR3 | Card/derived/meta-object ingress is supported; `QObject* q = card; q->deleteLater()` and qualified/member-pointer base calls are checker bypasses with `card_delete_bypass == 0` before PR7; `Card::event(DeferredDelete)` is only a delivered-event defense; ordinary non-Card QObject timing unchanged |
...
- [x] 1. PR1 - Characterize hazards and freeze finite ownership ledgers
  ...
  - Regression-first proof: add child-process characterization for stale alias UAF, owning-wrapper double-delete risk, adoption-after-delete request, worker deferred-delete accumulation, and direct owner destruction. `card-lifetime-legacy-red` is a dispatcher child suite, not a direct CTest. The `qsanguosha_card_lifetime` parent runs it in `ObserveOnly`, single-process, timeout `15s` and normalizes unsafe outcomes to internal `classification=RED`／`normalized_exit=70` JSON. PR1 expects that classification and returns 0; PR2 first changes the expectation to SAFE and captures the resulting nonzero red evidence, then implementation makes child and parent exit 0 with exact `Lua error: attempt to use deleted Card`／`Lua error: attempt to delete Card with unknown ownership`, `actually_destroyed==1`, and `stale_access==1`. Unknown-suite exit 64 is tested separately and allocator crash is never required. Add dedicated characterization test proving canonical owner `Room::thread()` event dispatcher remains alive until Room teardown (`Room::stopGameThreads()` join / `Room::~Room()` entry) on ordinary/1v1/3v3/XMode paths, using a posted `BlockingQueuedConnection` probe and pending `deleteLater` delivery probe before join. If dispatcher-liveness proof fails, stop and do not substitute another owner; blocking gate.
...
- [x] 2. PR2 - Make every observed SWIG Card alias generation-safe
  ...
  - Regression-first proof: the registered dispatcher child `card-lifetime-legacy-red` is invoked only by core CTest `qsanguosha_card_lifetime`. PR1 parent expects internal `classification=RED`／`normalized_exit=70` and therefore returns 0. Before PR2 implementation, change the parent expectation to SAFE exact errors plus `actually_destroyed==1`／`stale_access==1` and capture its nonzero red result; after the SWIG/token layer, child and parent return 0 with those exact values. Unknown-suite exit 64 remains a separate dispatcher fixture. The green gate is `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure`.
  ...
  - Acceptance criteria: `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure` and `ctest --test-dir builds/cmake-vs2026 -C Debug --output-on-failure` both exit 0; named Lua Card lifetime CTest `qsanguosha_card_lifetime` 及 `qsanguosha_card_lifetime:lua` 通過（鎖定名稱，統一 regex `^qsanguosha_card_lifetime(:.*)?$`，所有命令固定 `ctest --test-dir builds/cmake-vs2026 -C Debug ...`）; generated `builds/cmake-vs2026/generated/sanguosha_wrap.cxx` contains all three checked dispatches after the project block and `git diff -- swig/sanguosha_wrap.cxx` empty; tests cover owner=0/1, derived/base/static-`QObject` aliases, `CardList`/MustGetPtr, lightuserdata, deterministic address reuse, two runtimes, destruction racing first observation (destructor wins => no wrapper), Lua-invisible side-table authority, outermost-pcall pin surviving inner `lua_error`, and paused conversion/use; `ObserveOnly` physical timing unchanged and unobserved-card token count stays zero; **no `debug\\QSanguosha.exe --lua-test` gate**; PR2 still diagnostics/classification only, physical reclaim assertions remain at PR5/PR6. Every PR2 test constructs mode explicitly.
...
- [x] 3. PR3 - Route deletion ingress through provenance-aware legacy-exact APIs
  ...
  - Regression-first proof: add native Card/SkillCard/derived/meta-object ingress（受支援入口），`QObject* q = card; q->deleteLater()`／qualified base-call 視為 bypass 的 checker 測試，ordinary non-Card QObject control, Parse return-after-request, definition/adopted no-op, exact unknown error, and four yjcm2023 failure-branch tests before implementation and retain red/green evidence.
...
- [x] 5. PR5 - Implement dormant reclamation, native leases, and transactional WrappedCard adoption (T4)
...
- [x] 7. PR7 - Enable managed reclamation through one reversible default switch (T4 pre-gate)
...
- [ ] F2. Code quality and concurrency review (T4)
  - Review the full diff and relevant unchanged callers for token linearization, Lua-invisible side-table authority, destruction-won wrapper refusal, outermost-pcall conversion pins (no `lua_error`-skippable RAII), manager/Lua lock order, Qt affinity/move/BlockingQueuedConnection safety to canonical owner `Room::thread()`, 承認 `QObject::deleteLater()` 非 virtual 且 worker 無 event loop（支援入口鎖定 Card*／衍生／meta-object，QObject*／qualified 為 bypass，Card::event 僅 dispatcher 投遞時防線），adoption failure atomicity (all recoverable before move, no-fail after move, target lease before move, setup normalization), lease copy/move/release, shutdown order（第 7 步顯式冪等清除，第 8 步後唯一 FINAL_GAUGE），ABI/source compatibility, and bounded storage. Run `cmake --preset vs2026-x64` (if needed) + `cmake --build --preset debug --parallel 8` + `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure` + `ctest --test-dir builds/cmake-vs2026 -C Debug --output-on-failure`（鎖定 `qsanguosha_card_lifetime`／`qsanguosha_card_lifetime:lua`／`qsanguosha_card_lifetime:synthetic-30`／`qsanguosha_card_lifetime:synthetic-50`，統一 regex `^qsanguosha_card_lifetime(:.*)?$`，所有命令 `ctest --test-dir builds/cmake-vs2026 -C Debug ...`）。Output APPROVE only if diagnostics are clean, checker 說明 AST／pattern 能力與盲區且不以 scan clean 單獨宣稱閉合，並確認 no manager lock encloses Lua/Qt/Card work. Evidence: `<attemptDir>/final-F2-code-quality.{md,txt}`.
- [ ] F3. Real manual QA (T4)
  - Use the built Debug artifact, not mocks alone: run `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure`（鎖定 `qsanguosha_card_lifetime`／`qsanguosha_card_lifetime:lua`／`qsanguosha_card_lifetime:synthetic-30`／`qsanguosha_card_lifetime:synthetic-50`，統一 regex `^qsanguosha_card_lifetime(:.*)?$`，所有命令 `ctest --test-dir builds/cmake-vs2026 -C Debug ...`，取代已移除的 `debug\QSanguosha.exe --lua-test`）、exact N6 deterministic stress rows（單一 domain／owner thread／barrier／四操作，per-epoch `managed_live<=actor_count && pending<=actor_count && wrapper_leases<=2*actor_count` 且 drain 後零，且 FINAL_GAUGE 僅第 8 步後）、the pre-PR7 `RoomState::reset`/`resetCard` integration test, and the exact `python tools/autotest/headless_runner.py --exe <Debug exe> --modes 20p --games 5 --parallel 1 --seed 2026082201` headless command. Inspect the generated wrapper, headless log/CSV（含每局唯一 JSON final-gauge marker 校驗：僅第 8 步後唯一，缺失／重複／非零即失敗）、stale/unknown exact error text (`Lua error: attempt to use deleted Card`／`Lua error: attempt to delete Card with unknown ownership`／`Card lifetime error: rejected opaque QVariant Card payload`)、destruction-affinity traces (canonical owner `Room::thread()`), PR1 dispatcher-liveness evidence (4 variants), and final counter snapshots. Output APPROVE only for 5/5 games, no crash/UAF/double delete, zero worker-affinity adopted survivors, marker 僅第 8 步且校驗通過，且 every Room/runtime gauge at baseline. Evidence: `<attemptDir>/final-F3-manual-qa.{md,log,csv,json}`.
...
**演練 2 — 單獨回退 PR7（保留 PR8）＋全量驗證後再全回退：** 於另一隔離 worktree 僅回退 PR7（`defaultCardLifetimeMode()` 回 `ObserveOnly`，保留 PR8 stress/文件），重跑：`cmake --build --preset debug --parallel 8`、`ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure`（focused，鎖定 `qsanguosha_card_lifetime`／`qsanguosha_card_lifetime:lua`／`qsanguosha_card_lifetime:synthetic-30`／`qsanguosha_card_lifetime:synthetic-50`，統一 regex）、`ctest --test-dir builds/cmake-vs2026 -C Debug --output-on-failure` 全量、`ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime:synthetic-30$" --output-on-failure` 及 `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime:synthetic-50$" --output-on-failure` stress（含 per-epoch `managed_live<=actor_count && pending<=actor_count && wrapper_leases<=2*actor_count` 且 drain 後零）、`python tools/autotest/headless_runner.py --exe <Debug exe> --modes 20p --games 5 --parallel 1 --seed 2026082201` 的 ObserveOnly 20p（5/5 完成、零失敗、JSON marker 缺失/重複/非零即失敗，marker 僅第 8 步後）。驗證 PR1–6/8 綠態且預設行為回退；其後於**同一 worktree** 接續依序回退 PR8→PR6→PR5→PR4→PR3→PR2→PR1 完成全回退至基線。每步同樣檢查 `git diff -- swig/sanguosha_wrap.cxx` 為空且無產品程式碼殘留。
...
- Except the unique PR7 selector-default regression, every lifetime test explicitly selects mode. `cmake --preset vs2026-x64` (if needed) + `cmake --build --preset debug --parallel 8` + `ctest --test-dir builds/cmake-vs2026 -C Debug -R "^qsanguosha_card_lifetime(:.*)?$" --output-on-failure` (鎖定 `qsanguosha_card_lifetime`／`qsanguosha_card_lifetime:lua`／`qsanguosha_card_lifetime:synthetic-30`／`qsanguosha_card_lifetime:synthetic-50`，統一 regex `^qsanguosha_card_lifetime(:.*)?$`，所有命令 `ctest --test-dir builds/cmake-vs2026 -C Debug ...`，取代已移除的 `debug\QSanguosha.exe --lua-test`) + `ctest --test-dir builds/cmake-vs2026 -C Debug --output-on-failure` + source checker + generated-wrapper inspection and F1-F4 all approve on the same final SHA. `git diff --check` passes; tracked `swig/sanguosha_wrap.cxx`, legacy extensions, `lua/ai`, third-party `include/`/`lib/`, and production mode registry remain untouched. Per-epoch `managed_live<=actor_count && pending<=actor_count && wrapper_leases<=2*actor_count` and drain-after-zero, plus per-game unique JSON final-gauge marker（僅第 8 步 final-zero assertion 後唯一輸出，提前於 postclose／第 6 步為違規，missing/duplicate/non-zero fails PR8 CSV), are part of the success gate.
